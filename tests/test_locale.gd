...
const ZH_HANT_GLOSSARY_SAMPLES: Dictionary = {
	...
	"ui.help.vigilBody": "一切皆不白費。每段朝聖之路開始時，<b>守爐人</b>贈予恩賜並讓你選擇提燈術。倒下時，將一事刻入石中——一座<b>紀念碑</b>，下一段旅程可在同一幕取回。每次碎裂、燃燼與擊殺皆累積畢生<b>功績</b>，解鎖新牌、遺物，與第二面向——<b>灰衛</b>。抵達破曉一次後，<b>誓言</b>開啟：一條可選的嚴酷難度之梯，留給願意踏上更殘酷朝聖之路的人。",
...
static func _persistence_calls_and_shell(fails: Array[String]) -> void:
	...
	_check(fails, call_keys.size() == 53,
		"expected 53 save-error call sites, found %d" % call_keys.size())
