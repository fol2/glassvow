...
class GodotRuntimeProfileContractTests(unittest.TestCase):
    ...
    def test_hid_input_probe_grammar_expands_only_frozen_g0_parents(self) -> None:
        paths = self.profile["accessGrammar"]["paths"]
        hid = "/sys/devices/0006:045E:0621.0001/input"
        serio = (
        ...
        self.assertIn(f"P\topenat\t2752512\t{f'{hid}/input0'.encode().hex()}", actual)
        self.assertIn(
            f"P\topenat\t2621696\t{f'{hid}/input0/event0'.encode().hex()}",
            actual)
        self.assertIn(f"P\topenat\t2752512\t{f'{serio}/input1'.encode().hex()}", actual)
        self.assertIn(
