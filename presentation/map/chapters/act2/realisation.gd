...
static func finish(source: MapLayoutResult, landscape: Node3D) -> Dictionary:
	...
	for measured: Dictionary in landscape.measured_placements:
		...
		if measured["terminal"]: data["hero_placements"]["terminus"]=row
			...
			row["semantic_zone"]="drowned-city-surround"
	...
	if not camera["ok"]: return {"ok":false,"reason":"City surface cannot frame legal choices: "+JSON.stringify(camera["failures"])}
	...
	return {"ok":result!=null,"reason":"Invalid city surface record" if result==null else "","result":result,"assets":assets,"framing":framing,"version":VERSION}
