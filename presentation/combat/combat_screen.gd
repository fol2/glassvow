...
func _build_ui() -> void:
	...
	bg.color = Color(0.055, 0.06, 0.09)
	...
	player_box.anchor_top = 1.0
	player_box.anchor_bottom = 1.0
	player_box.offset_left = 16
	player_box.offset_top = -300
	player_box.offset_right = 220
	player_box.offset_bottom = -16
	player_box.add_theme_constant_override("separation", 8)
	add_child(player_box)
	player_box.add_child(_label("The Duskblade"))
	_player_hp = _label("")
	player_box.add_child(_player_hp)
	_player_ward = _label("")
	player_box.add_child(_player_ward)
	...
	_embers_label = _label("")
	player_box.add_child(_embers_label)
	...
	_end_turn.custom_minimum_size = Vector2(0, 52)
