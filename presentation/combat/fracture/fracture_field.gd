...
## The shortest arm worth generating, as a body fraction. Derived from the
...
## This is what makes the radial COUNT derived — the reference authors four arms and
## six for a big hit; here the energy decides how many arms it can afford.
...
func strike(net: CrackNet, blow: Blow) -> Array[Dictionary]:
	...
	if blow.energy < MIN_ARM:
		return out
	# Derived, not authored: how many arms the delivered energy can afford at a
	# length each that is long enough to see.
	var arms: int = clampi(int(blow.energy / MIN_ARM), 1, MAX_ARMS)
	var each: float = blow.energy / float(arms)
	...
	for i: int in range(arms):
		...
		pending.append([blow.at, Vector2(cos(a), sin(a)), each])
