extends SceneTree
## Deterministic, research-only legal combat probe for issue #525.
...
func _initialize() -> void:
	...
	if loaded.has("error"):
		push_error("mechanism_probe: %s" % str(loaded["error"]))
		quit(2)
		return
	var content: ContentDB = BalanceCatalogue.load_prepared(loaded)
	if content == null:
		push_error("mechanism_probe: content did not load a catalogue")
		quit(2)
		return
	var rows: Array = []
