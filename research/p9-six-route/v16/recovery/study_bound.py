...
ENGINE=Path('/mnt/data/p9_engine/Godot_v4.7.2-stable_linux.x86_64')
...
def bound_observer(specs):
    obs=observer()
    obs['controller_sha256']=sha(__file__)
    obs['content_files']={s['content_path']:sha(s['content_path']) for s in specs}
    return obs
...
def execute(spec,folder,obs,timeout):
    ...
    expected_content=obs['content_files'][spec['content_path']]
    if sha(spec['content_path'])!=expected_content:raise ValueError('Content differs from frozen bytes: '+key)
    if receipt.exists():
        old=json.loads(receipt.read_text())
        if old['spec']==spec and old['observer']==obs and old['complete'] and old['output_sha256']==sha(out) and old['log_sha256']==sha(log) and old['content_sha256']==expected_content and old['config_sha256']==sha(cfg):return old
...
def batch(name,specs,workers=4,timeout=600):
    obs=bound_observer(specs);assert obs['engine']==ENGINE_HASH
    ...
    results=[]
    # Keep only a worker-sized frontier; a failed cell stops future dispatch.
    # The frozen assignment stays intact; partial completion is not a summary.
    remaining=iter(specs)
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        pending={pool.submit(execute,s,folder,obs,timeout) for s in [next(remaining,None) for _ in range(workers)] if s is not None}
        while pending:
            done,pending=concurrent.futures.wait(pending,return_when=concurrent.futures.FIRST_COMPLETED)
            errors=[task.exception() for task in done if task.exception() is not None]
            if errors:
                for task in pending:task.cancel()
                write(folder/'INCOMPLETE.json',{'status':'INCOMPLETE_NOT_P9','error':repr(errors[0]),'completed_before_failure':len(results),'assigned_cells':len(specs)})
                raise errors[0]
            results.extend(task.result() for task in done)
            for _ in done:
                spec=next(remaining,None)
                if spec is not None:pending.add(pool.submit(execute,spec,folder,obs,timeout))
    assert bound_observer(specs)==obs,'Source or content changed during batch'
