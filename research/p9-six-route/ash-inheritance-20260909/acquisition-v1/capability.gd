					...
					for id_v: Variant in db.cards:
						...
						for up: bool in [false, true]:
							...
							if not random_build and aspect == 1:
								if id == "bloodRite":
									expected += float(policies[pi].status.venomousAsh)
									if deck_kind in ["consumer", "duplicate"]: expected += 2.0 * float(policies[pi].card.aspectBonus)
								if id == "leechBlade" and deck_kind == "source": expected += 2.0 * float(policies[pi].card.aspectBonus)
