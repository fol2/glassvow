"""Inspect incomplete text-encoded transport; no native execution or admission.
The first inspector wrongly treated base64 text as XZ bytes; no data was changed.
"""
...
for i,want in enumerate(EXPECTED):
 ...
 parts.append(b)
encoded=b''.join(parts).replace(b'\n',b'')
compressed=base64.b64decode(encoded,validate=True)
dec=lzma.LZMADecompressor();raw=dec.decompress(compressed)
# The end of an incomplete stream may split a UTF-8 code point, never an interior one.
import codecs
...
result={'kind':'INCOMPLETE_TRANSPORT_PREFIX_DIAGNOSTIC','part_bytes':[len(b) for b in parts],'compressed_bytes':len(compressed),'raw_prefix_bytes':len(raw),'xz_complete':dec.eof,'complete_nodes':len(nodes),'nodes_array_closed':pos<len(text) and text[pos]==']','sequence_nodes_by_source':counts,'tail_after_nodes':text[pos:pos+5000],'prefix_tail':text[-1200:],'native_runs':0,'remote_packet_complete':False}
