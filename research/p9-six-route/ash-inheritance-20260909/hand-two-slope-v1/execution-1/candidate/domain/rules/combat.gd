...
# Research-only intervention switches; not promoted product configuration.
...
func preview_play(
	...
	for fx_v: Variant in effects:
		...
		if kind == "dmg":
			...
			var sid: String = str(fx.get("id", ""))
			if sid == "leech" and bloodfire_enabled and bloodfire_consumer_enabled and run != null and run.aspect == 1 and _sget(p.statuses, "bloodfire") > 0:
