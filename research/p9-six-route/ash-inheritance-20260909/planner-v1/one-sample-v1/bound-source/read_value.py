...
def validate_extra(rows,method):
 for row in rows:
  ...
  if method=='stock':require(row['query_count']==row['root_rollouts']==0 and not row['decisions'],'STOCK_DISPATCH_CHANGED')
   ...
   require(all(d['readonly'] is True and isinstance(d.get('alternatives'),list) and len(d['alternatives'])>0 and d['root_rollouts']==len(d['alternatives']) for d in row['decisions']),'QUERY_ISOLATION')
