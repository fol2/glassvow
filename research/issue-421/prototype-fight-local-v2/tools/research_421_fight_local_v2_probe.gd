extends SceneTree
## Deterministic categorical-encoding probe for issue #421 fight-local sequencing.
...
func _scripted(content: ContentDB, spec: Dictionary) -> Dictionary:
	...
	var run: RunState = RunState.new_run(content,
		int(float(str(spec.get("seed", 347601)))),
...
func _fail(message: String) -> void:
	push_error("research_421_fight_local_v2_probe: %s" % message)
	quit(2)
