...
static func _harvest_fight(game: GlassvowGame) -> void:
	...
	for event_v: Variant in game.cb.queue:
		...
		elif kind == "research421PositiveOverkill":
			var stage: String = str(event.get("stage", ""))
			_bump("positiveOverkill%s" % stage.to_pascal_case())
			if stage == "producer":
				_bump("positiveOverkillAmount", _ji(event.get("overkill", 0)))
			elif stage == "expiry":
				var reason: String = str(event.get("reason", ""))
				var marks: int = _ji(event.get("eligibleMarks", 0))
				_bump("positiveOverkillExpiry%s" % reason.to_pascal_case())
				if reason == "consumer-disabled":
					_bump("positiveOverkillEligibleCards")
					_bump("positiveOverkillEligibleMarks", marks)
					_bump("positiveOverkillEligibleCard_%s" % last_play)
					if marks > 1:
						_bump("positiveOverkillMultiMarkCards")
				elif reason == "combat-over":
					_bump("positiveOverkillFinalMarks", marks)
			elif stage == "payoff":
				_bump("positiveOverkillRequested", _ji(event.get("requested", 0)))
				_bump("positiveOverkillRealised", _ji(event.get("realised", 0)))
		elif kind == "status":
