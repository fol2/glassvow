...
static var _research421_excess_producer: bool = false
static var _research421_excess_consumer: bool = false
...
static func configure_research421_positive_overkill(
	producer_enabled: bool, consumer_enabled: bool
) -> void:
	_research421_excess_producer = producer_enabled
	_research421_excess_consumer = consumer_enabled
...
func hit_enemy(
	...
	if cb.pending_chips_active and is_attack and loss > 0:
		...
		rec["hit"] = true
		var overkill: int = 0 if handoff else maxi(0, -e.hp)
		if _research421_excess_producer and run.aspect == 0 and overkill > 0:
			rec["research421PositiveOverkillIntrinsic"] = true
			cb.queue.append({
				"t": &"research421PositiveOverkill", "stage": "producer",
				"idx": e.idx, "amount": loss, "blocked": blocked,
				"hpAfter": maxi(0, e.hp), "killingBlow": true,
				"overkill": overkill,
			})
			cb.queue.append({
				"t": &"research421PositiveOverkill", "stage": "mediator-set",
...
func play_card(run: RunState, cb: CombatState, uid: int, target_idx: Variant = null) -> bool:
	...
	var research421_excess_marks: int = 0
	if _research421_excess_producer and run.aspect == 0:
		for idx_v: Variant in cb.pending_chips.keys():
			var idx: int = idx_v
			var rec: Dictionary = cb.pending_chips[idx]
			if rec.get("research421PositiveOverkillIntrinsic", false) == true \
					and cb.enemies[idx].hp <= 0:
				research421_excess_marks += 1
	if cb.pending_chips_active and not cb.over:
		...
		if research421_excess_marks > 0 and _research421_excess_consumer:
			cb.queue.append({
				"t": &"research421PositiveOverkill", "stage": "consumer",
				"eligibleMarks": research421_excess_marks,
			})
			var realised: int = gain_embers(run, cb, 1)
			cb.queue.append({
				"t": &"research421PositiveOverkill", "stage": "payoff",
				"requested": 1, "realised": realised,
			})
	if research421_excess_marks > 0:
		var expiry_reason: String = "consumed" \
			if _research421_excess_consumer and not cb.over else \
			("combat-over" if cb.over else "consumer-disabled")
		cb.queue.append({
			"t": &"research421PositiveOverkill", "stage": "expiry",
			"eligibleMarks": research421_excess_marks, "reason": expiry_reason,
		})
