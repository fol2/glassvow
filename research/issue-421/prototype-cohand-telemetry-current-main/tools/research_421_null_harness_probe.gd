extends SceneTree
## Instrumented current-main whole-run probe for issue #421 null-harness identity.
...
func _initialize() -> void:
	...
	if opts.has("error"):
		push_error("research_421_null_harness_probe: %s" % str(opts["error"]))
		quit(2)
		return
	var raw: Variant = JSON.parse_string(FileAccess.get_file_as_string(str(opts["plan"])))
	if typeof(raw) != TYPE_DICTIONARY:
		push_error("research_421_null_harness_probe: plan must be a dictionary")
		quit(2)
		return
	var plan: Dictionary = raw
	var loaded: Dictionary = BalanceCatalogue.open({"content": str(plan.get("content", ""))})
	if loaded.has("error"):
		push_error("research_421_null_harness_probe: %s" % str(loaded["error"]))
		quit(2)
		return
	var content: ContentDB = BalanceCatalogue.load_prepared(loaded)
	if content == null:
		push_error("research_421_null_harness_probe: content did not load")
		quit(2)
		return
	var rows_v: Variant = plan.get("rows", [])
	if typeof(rows_v) != TYPE_ARRAY:
		push_error("research_421_null_harness_probe: rows must be an array")
		quit(2)
		return
	var rows: Array[Dictionary] = []
	for spec_v: Variant in rows_v:
		if typeof(spec_v) != TYPE_DICTIONARY:
			push_error("research_421_null_harness_probe: every row must be a dictionary")
			quit(2)
			return
		var spec: Dictionary = spec_v
		var sampled: Array[Dictionary] = Policy.sample_range(
			int(float(str(spec["policyRoot"]))), int(float(str(spec["policyIndex"]))), 1)
		var research_v: Variant = spec.get(
			"research421", {"schemaVersion": 1}
			if spec.get("explicitNull", false) == true else {})
		if typeof(research_v) != TYPE_DICTIONARY:
			push_error("research_421_null_harness_probe: research421 must be a dictionary")
			quit(2)
			return
		...
		if str(row.get("error", "")) == \
				"research421 accepts only the explicit null schemaVersion=1":
			push_error("research_421_null_harness_probe: %s" % row["error"])
			quit(2)
			return
	...
	if file == null:
		push_error("research_421_null_harness_probe: cannot write output")
		quit(2)
		return
