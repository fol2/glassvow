...
static func simulate(content: ContentDB, aspect: String, seed: int, vow: int = 0,
	...
	if _research_trace.get("capture", false) == true:
		...
		_research_trace["turns"] = []
		_research_trace["draws"] = []
		_research_trace["plays"] = []
		_research_trace["dies"] = []
		_research_trace["reshuffles"] = []
		_research_trace["kindles"] = []
		_research_trace["cardRewards"] = []
		_research_trace["bossRelics"] = []
	var research_fault: String = _validate_research421(research421)
...
static func _validate_research421(settings: Dictionary) -> String:
	if settings.is_empty():
		return ""
	if settings.size() != 1 or not settings.has("schemaVersion") \
			or typeof(settings["schemaVersion"]) != TYPE_INT \
			or int(float(str(settings["schemaVersion"]))) != 1:
		return "research421 accepts only the explicit null schemaVersion=1"
	return ""
...
static func _harvest_fight(game: GlassvowGame, fight_index: int) -> void:
	...
	for event_v: Variant in game.cb.queue:
		...
		if kind == "turn":
			_trace_append("turns", {"fight": fight_index, "event": event_index,
				"n": _ji(event.get("n", -1))})
			last_play = ""
			if relics.has("hollowCrown"):
			...
			_trace_append("reshuffles", {"fight": fight_index, "event": event_index,
				"n": _ji(event.get("n", -1))})
		elif kind == "draw":
			var drawn: String = str(event.get("id", ""))
			_trace_append("draws", {"fight": fight_index, "event": event_index,
				"id": drawn, "uid": _ji(event.get("uid", -1))})
			_bump("%sDrawn" % drawn)
			...
			_trace_append("plays", {"fight": fight_index, "event": event_index,
				...
				"targetIdx": event.get("targetIdx")})
			_bump("%sPlayed" % last_play)
		elif kind == "die":
			_trace_append("dies", {"fight": fight_index, "event": event_index,
				"idx": _ji(event.get("idx", -1))})
		elif kind == "kindle":
			_trace_append("kindles", {"fight": fight_index, "event": event_index,
				"id": str(event.get("id", "")), "uid": _ji(event.get("uid", -1))})
		elif kind == "status":
