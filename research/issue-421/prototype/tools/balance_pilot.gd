...
static func _pick_play(game: GlassvowGame) -> Dictionary:
	...
	for card: CardInst in game.cb.hand:
		...
		if lethal and best_score >= 10000.0:
			continue
		if not _advances_fight(d, dusk) and (block <= 0 or unblocked <= 0):
			continue
...
static func _combat_score(game: GlassvowGame, card: CardInst, d: Dictionary, target: Variant,
	...
	return score
static func _hand_has(game: GlassvowGame, id: StringName) -> bool:
	...
	if _special_id(d) == "catalyst":
		...
		for e: EnemyCombatant in living:
			if int(float(str(e.statuses.get("poison", 0)))) \
				...
				stacked = e
		return stacked.idx
	if game.run.aspect == 0 and _special_id(d) == "execute":
		for e: EnemyCombatant in living:
			if int(float(str(e.statuses.get("scoreline", 0)))) > 0:
				return e.idx
...
static func card_score(d: Dictionary, aspect: int, card_id: String = "") -> float:
	...
	if dusk and card_id in ["eclipseSlash", "chisel", "warCry", "limitBreak",
			"resonantLance", "executioner", "quickSlash", "fortify"]:
		score += _w("card", "aspectBonus")
...
static func _status_value(id: String, n: int, dusk: bool) -> float:
	match id:
		...
		"bloodfire":
			return 0.0 if dusk else _w("status", "venomousAsh")
...
static func _special_id(d: Dictionary) -> String:
	...
	return ""
static func _advances_fight(d: Dictionary, dusk: bool) -> bool:
	if str(d.get("type", "")) in ["attack", "power"]:
		return true
	for fx_v: Variant in d.get("effects", []):
		var fx: Dictionary = fx_v
		if dusk and str(fx.get("kind", "")) == "status" \
				and str(fx.get("id", "")) == "afterimage":
			return true
...
static func choose_card(ids: Array, content: ContentDB, aspect: int, rng: Rng = null,
	...
	for id_v: Variant in ids:
		...
		var candidate: float = card_reward_score(definition, aspect, id, deck)
		if candidate > score:
			best = id
			score = candidate
	return best
