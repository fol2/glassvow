...
static var _capture_upgrades: bool = false
static var _upgrade_trace: Array[Dictionary] = []
...
static func simulate(content: ContentDB, aspect: String, seed: int, vow: int = 0,
		...
		capture_upgrades: bool = false) -> Dictionary:
	_probe = {}
	_capture_upgrades = capture_upgrades
	_upgrade_trace = []
	Pilot.set_ban(ban)
	...
	match node.type:
		"rest":
			if game.run.player.hp * 100 <= game.run.player.max_hp * Pilot._wi("restHpPct"):
				...
				_upgrade_best(game, "rest")
		"event": _resolve_event(game)
		"shop": _resolve_shop(game)
		"treasure": _claim_treasure(game)
static func _upgrade_best(game: GlassvowGame, source: String) -> void:
	...
	if best != null:
		best.up = true
		if _capture_upgrades:
			_upgrade_trace.append({"source": source, "act": game.run.act,
				"uid": best.uid, "id": String(best.id)})
static func _resolve_event(game: GlassvowGame) -> void:
	...
	match str(pending.get("kind", "")):
		...
		"upgrade": _upgrade_best(game, "event")
		"remove":
...
static func _result(run: RunState, aspect: String, seed: int, outcome: String,
	...
	if _capture_upgrades:
		row["trajectory"] = {"upgrades": _upgrade_trace.duplicate(true)}
	return row
