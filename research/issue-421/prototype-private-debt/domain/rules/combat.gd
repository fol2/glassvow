...
func _apply_effect(
	...
	var aspect_v: Variant = fx.get("aspect")
	if aspect_v != null and run.aspect != _ji(aspect_v):
		return
	var p: PlayerCombatant = cb.player
	...
	match kind:
		...
		"removeCardFromHand":
			var remove_id: StringName = StringName(str(fx["id"]))
			for hand_index: int in range(cb.hand.size()):
				var held: CardInst = cb.hand[hand_index]
				if held.id == remove_id:
					cb.hand.remove_at(hand_index)
					cb.queue.append({
						"t": &"removeCardFromHand",
						"id": String(held.id),
						"uid": held.uid,
					})
					break
