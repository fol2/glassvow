...
static var _research421_precision_producer: bool = false
static var _research421_precision_consumer: bool = false
...
static func configure_research421_terminal_hit_precision(
	producer_enabled: bool, consumer_enabled: bool
) -> void:
	_research421_precision_producer = producer_enabled
	_research421_precision_consumer = consumer_enabled
...
func hit_enemy(
	...
	if cb.pending_chips_active and is_attack and loss > 0:
		...
		rec["hit"] = true
		if _research421_precision_producer and run.aspect == 0 and not handoff \
				and e.hp <= 0 and maxi(0, -e.hp) == 0:
			rec["research421ExactLethalIntrinsic"] = true
			cb.queue.append({
				"t": &"research421TerminalHitPrecision", "stage": "producer",
				"idx": e.idx, "amount": loss, "blocked": blocked,
				"hpAfter": maxi(0, e.hp), "killingBlow": true, "overkill": 0,
			})
			cb.queue.append({
				"t": &"research421TerminalHitPrecision", "stage": "mediator-set",
				"idx": e.idx, "intrinsic": 1,
			})
...
func play_card(run: RunState, cb: CombatState, uid: int, target_idx: Variant = null) -> bool:
	...
	var research421_precision_marks: int = 0
	if _research421_precision_producer and run.aspect == 0:
		for idx_v: Variant in cb.pending_chips.keys():
			var idx: int = idx_v
			var rec: Dictionary = cb.pending_chips[idx]
			if rec.get("research421ExactLethalIntrinsic", false) == true \
					and cb.enemies[idx].hp <= 0:
				research421_precision_marks += 1
	if cb.pending_chips_active and not cb.over:
		...
		if research421_precision_marks > 0 and _research421_precision_consumer:
			cb.queue.append({
				"t": &"research421TerminalHitPrecision", "stage": "consumer",
				"eligibleMarks": research421_precision_marks,
			})
			var realised: int = gain_embers(run, cb, 1)
			cb.queue.append({
				"t": &"research421TerminalHitPrecision", "stage": "payoff",
				"requested": 1, "realised": realised,
			})
	if research421_precision_marks > 0:
		var expiry_reason: String = "consumed" \
			if _research421_precision_consumer and not cb.over else \
			("combat-over" if cb.over else "consumer-disabled")
		cb.queue.append({
			"t": &"research421TerminalHitPrecision", "stage": "expiry",
			"eligibleMarks": research421_precision_marks, "reason": expiry_reason,
		})
