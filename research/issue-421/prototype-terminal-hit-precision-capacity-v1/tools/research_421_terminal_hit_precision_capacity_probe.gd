extends SceneTree
## Fixed-policy whole-run capacity probe for issue #421 terminal-hit precision.
	...
	for spec_v: Variant in rows_v:
		...
		if configured:
			if not factor_available:
				_fail("precision interface unavailable")
				return
			rules.call(
...
func _fail(message: String) -> void:
	push_error("research_421_terminal_hit_precision_capacity_probe: %s" % message)
	quit(2)
