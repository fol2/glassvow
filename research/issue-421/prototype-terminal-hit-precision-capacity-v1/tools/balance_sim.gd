...
static func _harvest_fight(game: GlassvowGame) -> void:
	...
	for event_v: Variant in game.cb.queue:
		...
		elif kind == "research421TerminalHitPrecision":
			var stage: String = str(event.get("stage", ""))
			_bump("terminalHitPrecision%s" % stage.to_pascal_case())
			if stage == "expiry":
				var reason: String = str(event.get("reason", ""))
				var marks: int = _ji(event.get("eligibleMarks", 0))
				_bump("terminalHitPrecisionExpiry%s" % reason.to_pascal_case())
				if reason == "consumer-disabled":
					_bump("terminalHitPrecisionEligibleCards")
					_bump("terminalHitPrecisionEligibleMarks", marks)
					_bump("terminalHitPrecisionEligibleCard_%s" % last_play)
					if marks > 1:
						_bump("terminalHitPrecisionMultiMarkCards")
				elif reason == "combat-over":
					_bump("terminalHitPrecisionFinalMarks", marks)
			elif stage == "payoff":
				_bump("terminalHitPrecisionRequested", _ji(event.get("requested", 0)))
				_bump("terminalHitPrecisionRealised", _ji(event.get("realised", 0)))
		elif kind == "status":
