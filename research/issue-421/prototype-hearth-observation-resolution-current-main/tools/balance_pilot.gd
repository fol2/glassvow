...
static var _research421_hearth_priority: bool = false
static func set_modes(build: bool, play: bool) -> void:
	random_build = build
	random_play = play
static func set_research421_hearth_priority(enabled: bool) -> void:
	_research421_hearth_priority = enabled
...
static func choose_relic(ids: Array, content: ContentDB, aspect: int = 0, rng: Rng = null) -> String:
	if random_build and rng != null:
		...
		for id_v: Variant in ids:
			if not is_banned(str(id_v)):
				legal.append(str(id_v))
		var picked: String = "" if legal.is_empty() else legal[rng.pick_index(legal.size())]
		return "crownOfTheHearth" if _research421_hearth_priority \
				and legal.has("crownOfTheHearth") else picked
	var best: String = ""
	var score: int = -1
	for id_v: Variant in ids:
		var id: String = str(id_v)
		...
		if candidate > score:
			...
			score = candidate
	return "crownOfTheHearth" if _research421_hearth_priority \
			and ids.has("crownOfTheHearth") and not is_banned("crownOfTheHearth") else best
