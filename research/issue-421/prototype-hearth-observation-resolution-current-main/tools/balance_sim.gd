...
static func simulate(content: ContentDB, aspect: String, seed: int, vow: int = 0,
		...
		research_trace: Dictionary = {}, research_settings: Dictionary = {}) -> Dictionary:
	_probe = {}
	_research_trace = research_trace
	if not _research_trace.is_empty():
		_research_trace["hearthBranchEvents"] = []
		_research_trace["nodePath"] = []
		_research_trace["fightPath"] = []
	var settings_error: String = _apply_research421_settings(research_settings)
	...
	for _act: int in range(3):
		...
		while not map.is_finished():
			...
			_trace_node(run, node, i)
			_enter_node(run, node)
			if node.is_combat():
				CombatRules.set_research421_hearth_observation(
					_research_trace, fights.size())
				var fight: Dictionary = _fight(game, node)
				_trace_fight(game, fights.size(), fight)
				_harvest_fight(game)
				fights.append(fight)
				if fight["result"] != "win":
...
static func _trace_node(run: RunState, node: MapNode, index: int) -> void:
	if _research_trace.get("pathCapture", false) != true:
		return
	var rows_v: Variant = _research_trace.get("nodePath", [])
	...
	rows.append({
		...
		"rngBefore": run.rng_state(),
	})
	_research_trace["nodePath"] = rows


static func _trace_fight(
	game: GlassvowGame, fight_index: int, fight: Dictionary
) -> void:
	if _research_trace.get("pathCapture", false) != true:
		return
	var rows_v: Variant = _research_trace.get("fightPath", [])
	var rows: Array = rows_v if typeof(rows_v) == TYPE_ARRAY else []
	var scoreline_ready: bool = false
	var afterimage_ready: bool = false
	var scoreline_route: bool = false
	var afterimage_route: bool = false
	for event_v: Variant in game.cb.queue:
		if typeof(event_v) != TYPE_DICTIONARY:
			continue
		var event: Dictionary = event_v
		if str(event.get("t", "")) != "play":
			continue
		var card_id: String = str(event.get("id", ""))
		if card_id == "chisel":
			scoreline_ready = true
		elif card_id == "executioner" and scoreline_ready:
			scoreline_route = true
		if card_id == "defend":
			afterimage_ready = true
		elif card_id == "guardedStrike" and afterimage_ready:
			afterimage_route = true
	rows.append({
		"fight": fight_index,
		"result": str(fight.get("result", "")),
		"queueSha256": JSON.stringify(game.cb.queue).sha256_text(),
		"rngAfter": game.run.rng_state(),
		"scorelineRoute": scoreline_route,
		"afterimageRoute": afterimage_route,
	})
	_research_trace["fightPath"] = rows


static func _apply_research421_settings(settings: Dictionary) -> String:
	CombatRules.set_research421_hearth_payoff_per_ember(3)
	Pilot.set_research421_hearth_priority(false)
	for key_v: Variant in settings:
		if str(key_v) not in ["hearthPayoffPerEmber", "hearthAcquisitionPriority"]:
			return "unknown issue #421 research setting %s" % str(key_v)
	var payoff_v: Variant = settings.get("hearthPayoffPerEmber", 3)
	var priority_v: Variant = settings.get("hearthAcquisitionPriority", false)
	if typeof(payoff_v) != TYPE_INT:
		return "hearthPayoffPerEmber must be integer 0 or 3"
	var payoff: int = _ji(payoff_v)
	if payoff not in [0, 3]:
		return "hearthPayoffPerEmber must be integer 0 or 3"
	if typeof(priority_v) != TYPE_BOOL:
		return "hearthAcquisitionPriority must be boolean"
	CombatRules.set_research421_hearth_payoff_per_ember(payoff)
	Pilot.set_research421_hearth_priority(priority_v == true)
	return ""
