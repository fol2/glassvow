...
static var _research421_hold_producer: bool = false
static var _research421_hold_consumer: bool = false
...
static func configure_research421_cross_turn_hold(
	producer_enabled: bool, consumer_enabled: bool
) -> void:
	_research421_hold_producer = producer_enabled
	_research421_hold_consumer = consumer_enabled


static func _research421_hold_event(
	cb: CombatState, stage: String, values: Dictionary = {}
) -> void:
	var event: Dictionary = {"t": &"research421CrossTurnHold", "stage": stage}
	event.merge(values, true)
	cb.queue.append(event)


static func _research421_expire_hold(cb: CombatState, reason: String) -> int:
	if cb.research421_cross_turn_hold.is_empty():
		return 0
	var held: Dictionary = cb.research421_cross_turn_hold
	var uid: int = _ji(held.get("uid", 0))
	_research421_hold_event(cb, "expiry", {
		"uid": uid, "id": str(held.get("cardId", "")),
		"createdTurn": _ji(held.get("createdTurn", 0)),
		"turn": cb.turn, "reason": reason,
	})
	cb.research421_cross_turn_hold = {}
	return uid


func _research421_consume_hold(run: RunState, cb: CombatState, inst: CardInst) -> void:
	if cb.research421_cross_turn_hold.is_empty():
		return
	var held: Dictionary = cb.research421_cross_turn_hold
	if _ji(held.get("uid", 0)) != inst.uid:
		return
	if cb.turn != _ji(held.get("createdTurn", 0)) + 1:
		_research421_expire_hold(cb, "stale-turn")
		return
	if not _research421_hold_consumer:
		_research421_expire_hold(cb, "consumer-disabled")
		return
	var values: Dictionary = {
		"uid": inst.uid, "id": String(inst.id),
		"createdTurn": _ji(held.get("createdTurn", 0)), "turn": cb.turn,
	}
	_research421_hold_event(cb, "consumer", values)
	cb.research421_cross_turn_hold = {}
	_research421_hold_event(cb, "mediator-consume", values)
	var realised: int = gain_embers(run, cb, 1)
	_research421_hold_event(cb, "payoff", {
		"uid": inst.uid, "id": String(inst.id), "turn": cb.turn,
...
func _start_player_turn(run: RunState, cb: CombatState) -> void:
	...
	var draw_count: int = maxi(1, draws)
	if not cb.research421_cross_turn_hold.is_empty():
		var held: Dictionary = cb.research421_cross_turn_hold
		var held_uid: int = _ji(held.get("uid", 0))
		var held_present: bool = false
		for card: CardInst in cb.hand:
			if card.uid == held_uid:
				held_present = true
				break
		if cb.turn != _ji(held.get("createdTurn", 0)) + 1 or not held_present:
			_research421_expire_hold(cb, "stale-or-missing-at-turn-start")
		else:
			draw_count = maxi(0, draw_count - 1)
			_research421_hold_event(cb, "carry", {
				"uid": held_uid, "id": str(held.get("cardId", "")),
				"createdTurn": _ji(held.get("createdTurn", 0)),
				"turn": cb.turn, "drawReplacement": 1,
				"drawCount": draw_count,
			})
	draw_cards(run, cb, draw_count)
...
func lose_combat(run: RunState, cb: CombatState) -> void:
	_research421_expire_hold(cb, "defeat")
	cb.over = true
	cb.result = "loss"
	cb.player.hp = 0
	run.player.hp = 0
...
func _win_combat(run: RunState, cb: CombatState) -> void:
	_research421_expire_hold(cb, "victory")
	cb.over = true
	cb.result = "win"
...
func play_card(run: RunState, cb: CombatState, uid: int, target_idx: Variant = null) -> bool:
	...
	cb.queue.append({"t": EventTypes.ENERGY, "n": p.energy})
	_research421_consume_hold(run, cb, inst)
...
func end_turn(run: RunState, cb: CombatState) -> void:
	...
	var expired_uid: int = _research421_expire_hold(cb, "next-end-turn")
	var retained: CardInst = null
	if _research421_hold_producer and run.aspect == 0:
		for card: CardInst in cb.hand:
			if card.uid != expired_uid and str(card_data(card).get("type", "")) == "attack":
				retained = card
				break
	if retained != null:
		_research421_hold_event(cb, "producer", {
			"uid": retained.uid, "id": String(retained.id), "turn": cb.turn,
			"handIndex": cb.hand.find(retained),
		})
		cb.research421_cross_turn_hold = {
			"uid": retained.uid, "cardId": String(retained.id), "createdTurn": cb.turn,
		}
		_research421_hold_event(cb, "mediator-set", {
			"uid": retained.uid, "id": String(retained.id), "createdTurn": cb.turn,
		})
	# Discard every card except the sole retained mediator.
	var uids: Array = []
	var next_hand: Array[CardInst] = []
...
func kindle_from_hand(run: RunState, cb: CombatState, uid: int) -> bool:
	...
	if not can_kindle(run, cb, inst):
		return false
	if not cb.research421_cross_turn_hold.is_empty() \
			and _ji(cb.research421_cross_turn_hold.get("uid", 0)) == inst.uid:
		_research421_expire_hold(cb, "kindle")
