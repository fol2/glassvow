...
const RESEARCH421_SCORELINE_LEVELS: Array[int] = [0, 4, 6, 8]
const RESEARCH421_AFTERIMAGE_LEVELS: Array[int] = [0, 4, 5, 8]
...
static func configure_research421_fight_local(settings: Dictionary) -> String:
	...
	if settings.is_empty():
		return ""
	if not settings.has("schemaVersion") or typeof(settings["schemaVersion"]) != TYPE_INT \
			or int(float(str(settings["schemaVersion"]))) != 1:
		return "research421 requires schemaVersion=1"
	for key_v: Variant in settings.keys():
		var key: String = str(key_v)
		if key not in ["schemaVersion", "scorelineDamage", "afterimageWardCap"]:
			return "research421 unknown key %s" % key
	var scoreline: int = int(float(str(settings.get("scorelineDamage", 0))))
	var afterimage: int = int(float(str(settings.get("afterimageWardCap", 0))))
	if typeof(settings.get("scorelineDamage", 0)) != TYPE_INT \
			or scoreline not in RESEARCH421_SCORELINE_LEVELS:
		return "research421 scorelineDamage has unregistered level"
	if typeof(settings.get("afterimageWardCap", 0)) != TYPE_INT \
			or afterimage not in RESEARCH421_AFTERIMAGE_LEVELS:
		return "research421 afterimageWardCap has unregistered level"
