...
static var hearth_priority: bool = false
static func set_modes(build: bool, play: bool) -> void:
	random_build = build
	random_play = play
static func set_hearth_priority(enabled: bool) -> void:
	hearth_priority = enabled
...
static func choose_relic(ids: Array, content: ContentDB, aspect: int = 0, rng: Rng = null) -> String:
	if random_build and rng != null:
		...
		return "crownOfTheHearth" if hearth_priority \
				and legal.has("crownOfTheHearth") else picked
	if hearth_priority and ids.has("crownOfTheHearth") \
			and not is_banned("crownOfTheHearth"):
		return "crownOfTheHearth"
	var best: String = ""
	var score: int = -1
	for id_v: Variant in ids:
		var id: String = str(id_v)
