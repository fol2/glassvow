extends SceneTree
## Research-only direct and null-path probe for the Dusk acquisition contract.
...
func _initialize() -> void:
	...
	for spec_v: Variant in rows_v:
		...
		if acquisition not in ["", "off", "pilot"]:
			_fail("acquisition accepts only omitted, off or pilot")
			return
		if str(validation.get("mode", "")) == "whole" and acquisition == "pilot":
...
func _direct(content: ContentDB, spec: Dictionary) -> Dictionary:
	...
	Pilot.set_modes(random_build, false)
	...
	var choice: String = ""
	if aspect == "duskblade" and acquisition == "pilot":
		choice = Pilot.choose_dusk_package(content)
	...
	if reloaded == null:
		...
		return {}
	var policy: Dictionary = Pilot.policy_snapshot()
	return {
		"id": str(spec.get("id", "")),
		"aspect": aspect,
		...
		"choice": choice,
		"scorelineScore": Pilot.card_score(content.cards.get("executioner", {}), 0, "executioner"),
		"afterimageScore": Pilot.card_score(content.cards.get("guardedStrike", {}), 0, "guardedStrike"),
		"cardDecline": float(str(policy["cardDecline"])),
		"policy": policy,
...
func _fail(message: String) -> void:
	push_error("research_421_dusk_acquisition_probe: %s" % message)
	quit(2)
