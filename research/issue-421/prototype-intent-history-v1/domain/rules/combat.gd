...
static var _research421_intent_producer: bool = false
static var _research421_intent_consumer: bool = false

const _RESEARCH421_INTENT_KEY: String = "_research421IntentHistory"
...
static func configure_research421_intent_history(
	producer_enabled: bool, consumer_enabled: bool
) -> void:
	_research421_intent_producer = producer_enabled
	_research421_intent_consumer = consumer_enabled


func _research421_intent_event(
	cb: CombatState, stage: String, enemy: EnemyCombatant, values: Dictionary = {}
) -> void:
	var event: Dictionary = {
		"t": &"research421IntentHistory", "stage": stage,
		"idx": enemy.idx, "turn": cb.turn,
	}
	event.merge(values, true)
	cb.queue.append(event)


func _research421_expire_intent(
	cb: CombatState, enemy: EnemyCombatant, reason: String
) -> bool:
	if not enemy.flags.has(_RESEARCH421_INTENT_KEY):
		return false
	var record_v: Variant = enemy.flags.get(_RESEARCH421_INTENT_KEY)
	var values: Dictionary = {"reason": reason, "move": "", "createdTurn": -1}
	if typeof(record_v) == TYPE_DICTIONARY:
		var record: Dictionary = record_v
		values["move"] = str(record.get("move", ""))
		var created_v: Variant = record.get("createdTurn", -1)
		if typeof(created_v) == TYPE_INT:
			values["createdTurn"] = created_v
	enemy.flags.erase(_RESEARCH421_INTENT_KEY)
	_research421_intent_event(cb, "expiry", enemy, values)
	return true


func _research421_expire_all_intents(cb: CombatState, reason: String) -> void:
	for enemy: EnemyCombatant in cb.enemies:
		_research421_expire_intent(cb, enemy, reason)


func _research421_consume_intent(
	cb: CombatState, enemy: EnemyCombatant, inst: CardInst, card_type: String
) -> Dictionary:
	if enemy == null or card_type != "attack" \
			or not enemy.flags.has(_RESEARCH421_INTENT_KEY):
		return {}
	var record_v: Variant = enemy.flags.get(_RESEARCH421_INTENT_KEY)
	if typeof(record_v) != TYPE_DICTIONARY:
		_research421_expire_intent(cb, enemy, "malformed")
		return {}
	var record: Dictionary = record_v
	var move_v: Variant = record.get("move")
	var created_v: Variant = record.get("createdTurn")
	if typeof(move_v) != TYPE_STRING or typeof(created_v) != TYPE_INT \
			or str(move_v).is_empty():
		_research421_expire_intent(cb, enemy, "malformed")
		return {}
	if _ji(created_v) != cb.turn:
		_research421_expire_intent(cb, enemy, "stale-turn")
		return {}
	if str(move_v) != String(enemy.move_key):
		_research421_expire_intent(cb, enemy, "move-mismatch")
		return {}
	if not _research421_intent_consumer:
		_research421_expire_intent(cb, enemy, "consumer-disabled")
		return {}
	var values: Dictionary = {
		"move": str(move_v), "createdTurn": _ji(created_v),
		"uid": inst.uid, "id": String(inst.id),
	}
	_research421_intent_event(cb, "consumer", enemy, values)
	enemy.flags.erase(_RESEARCH421_INTENT_KEY)
	_research421_intent_event(cb, "mediator-consume", enemy, values)
	return {
		"idx": enemy.idx, "move": str(move_v), "createdTurn": _ji(created_v),
		"uid": inst.uid, "id": String(inst.id),
	}
...
func _compute_intents(run: RunState, cb: CombatState) -> void:
	for e: EnemyCombatant in cb.enemies:
		if e.hp <= 0:
			continue
		_research421_expire_intent(cb, e, "unanswered-window")
		...
		if _research421_intent_producer or _research421_intent_consumer:
			_research421_intent_event(cb, "ai-boundary", e, {
				"researchKeyPresent": e.flags.has(_RESEARCH421_INTENT_KEY),
				"oneBack": last, "twoBack": prev,
			})
		...
		if _research421_intent_producer and run.aspect == 0 \
				and not last.is_empty() and String(e.move_key) == last:
			_research421_intent_event(cb, "producer", e, {
				"move": String(e.move_key), "oneBack": last, "twoBack": prev,
			})
			e.flags[_RESEARCH421_INTENT_KEY] = {
				"move": String(e.move_key), "createdTurn": cb.turn + 1,
			}
			_research421_intent_event(cb, "mediator-set", e, {
				"move": String(e.move_key), "createdTurn": cb.turn + 1,
			})
...
func lose_combat(run: RunState, cb: CombatState) -> void:
	_research421_expire_all_intents(cb, "defeat")
	cb.over = true
	cb.result = "loss"
	cb.player.hp = 0
	run.player.hp = 0
...
func _win_combat(run: RunState, cb: CombatState) -> void:
	_research421_expire_all_intents(cb, "victory")
	cb.over = true
	cb.result = "win"
...
func _on_enemy_death(run: RunState, cb: CombatState, e: EnemyCombatant) -> void:
	_research421_expire_intent(cb, e, "target-death")
	e.hp = 0
...
func play_card(run: RunState, cb: CombatState, uid: int, target_idx: Variant = null) -> bool:
	...
	var research_intent_consumed: Dictionary = _research421_consume_intent(
		cb, target, inst, card_type
	)
	...
	if not research_intent_consumed.is_empty():
		var research_enemy: EnemyCombatant = cb.enemies[_ji(research_intent_consumed["idx"])]
		if cb.over:
			_research421_intent_event(cb, "expiry", research_enemy, {
				"reason": "combat-over-after-consume",
				"move": str(research_intent_consumed["move"]),
				"createdTurn": _ji(research_intent_consumed["createdTurn"]),
				"uid": _ji(research_intent_consumed["uid"]),
				"id": str(research_intent_consumed["id"]),
			})
		else:
			var realised: int = gain_embers(run, cb, 1)
			_research421_intent_event(cb, "payoff", research_enemy, {
				"move": str(research_intent_consumed["move"]),
				"createdTurn": _ji(research_intent_consumed["createdTurn"]),
				"uid": _ji(research_intent_consumed["uid"]),
				"id": str(research_intent_consumed["id"]),
				"requested": 1, "realised": realised,
			})
	return true
