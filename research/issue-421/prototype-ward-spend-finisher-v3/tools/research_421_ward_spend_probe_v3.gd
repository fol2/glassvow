extends SceneTree
## Engine-correct direct mediator probe for issue #421 Ward spend v3.
...
func _run_row(content: ContentDB, plan: Dictionary, spec: Dictionary) -> Dictionary:
	...
	var run: RunState = RunState.new_run(
		content, _int(spec, "seed"), "ward-spend-v3-%s" % str(spec["id"]), profile)
...
func _fail(message: String) -> void:
	push_error("research_421_ward_spend_probe_v3: %s" % message)
	quit(2)
