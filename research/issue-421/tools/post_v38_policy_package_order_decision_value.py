#!/usr/bin/env python3
"""Zero-row decision-value audit for policy-selective package ordering."""
...
PROTOCOL = core.ROOT / "protocols/post-v38-policy-package-order-decision-value-v1.json"
SUMMARY = core.ROOT / "summaries/post-v38-policy-package-order-decision-value-v1.json"
...
def main() -> None:
    ...
    for arm, arm_rows in arms.items():
        ...
        heldout.validate_rectangle(heldout_protocol, arm_rows)
    candidate_policy = heldout.split_rows(arms["structuralCandidate"])[1]
    null_policy = heldout.split_rows(arms["structuralNull"])[1]
