#!/usr/bin/env python3
"""Zero-row intent-history lifecycle design audit for issue #421."""
...
PROTOCOL = core.ROOT / "protocols/post-843e899-intent-history-design-v1.json"
SUMMARY = core.ROOT / "summaries/post-843e899-intent-history-design-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Intent-history design mismatch: {label}")
...
def classify(
    ...
    if not legal:
        return "futility", 2, "close-intent-history-at-zero-row-boundary", []
    if len(legal) == 1 or common.rank(legal[0]) < common.rank(legal[1]):
        return "success", 1, "freeze-repeat-one-back-attack-ember-contract-for-preflight", legal[:1]
    return "inconclusive", 3, "record-intent-history-design-inconclusive-at-cap", []
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite intent-history design summary")
    ...
    if elapsed > protocol["budget"]["maximumWallTimeSeconds"]:
        outcome, boundary, decision, selected, selected_ids = (
            "inconclusive", 3, "record-intent-history-design-inconclusive-at-cap", [], [],
        )
