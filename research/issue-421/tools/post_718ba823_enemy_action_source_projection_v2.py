...
BYTE_CAP = 90_000
WALL_CAP_SECONDS = 30
PROJECTED_MOVE_KEYS = {"intent", "dmg", "times", "block", "heal", "ramp", "fx", "addCards"}
OMITTED_MOVE_KEYS = {"name"}
ENCOUNTER_TIERS = {"weak", "normal", "elite", "boss"}
...
def clean_move(move: dict[str, Any]) -> dict[str, Any]:
    observed = set(move)
    require(observed <= PROJECTED_MOVE_KEYS | OMITTED_MOVE_KEYS, f"unprojected move keys: {sorted(observed - PROJECTED_MOVE_KEYS - OMITTED_MOVE_KEYS)}")
    require("intent" in move, "move missing intent")
    return {key: move[key] for key in sorted(PROJECTED_MOVE_KEYS) if key in move}


def project_content(content: dict[str, Any]) -> dict[str, Any]:
    enemies = content.get("enemies")
    encounters = content.get("encounters")
    require(isinstance(enemies, dict) and enemies, "enemies must be a non-empty object")
    require(isinstance(encounters, list) and encounters, "encounters must be a non-empty array")
    enemy_moves: dict[str, dict[str, Any]] = {}
    for enemy_id, enemy in sorted(enemies.items()):
        require(isinstance(enemy, dict), f"enemy must be an object: {enemy_id}")
        moves = enemy.get("moves")
        require(isinstance(moves, dict) and moves, f"enemy moves missing: {enemy_id}")
        enemy_moves[enemy_id] = {}
        for move_id, move in sorted(moves.items()):
            require(isinstance(move, dict), f"move must be an object: {enemy_id}/{move_id}")
            enemy_moves[enemy_id][move_id] = clean_move(move)

    multi_enemy_groups: list[dict[str, Any]] = []
    for act_index, act in enumerate(encounters):
        require(isinstance(act, dict) and set(act) == ENCOUNTER_TIERS, "encounter tier schema mismatch")
        for tier in sorted(act):
            require(isinstance(act[tier], list), f"encounter tier must be an array: {tier}")
            for group_index, group in enumerate(act[tier]):
                require(isinstance(group, list) and group, "encounter group must be a non-empty array")
                require(all(isinstance(enemy_id, str) and enemy_id in enemies for enemy_id in group), "unknown encounter enemy")
                if len(group) > 1:
    ...
    return {
        ...
        "projectedMoveKeys": sorted(PROJECTED_MOVE_KEYS),
        "omittedMoveKeys": sorted(OMITTED_MOVE_KEYS),
        "encounterTiers": sorted(ENCOUNTER_TIERS),
...
def enforce_caps(byte_count: int, elapsed_seconds: float) -> None:
    require(byte_count <= BYTE_CAP, "projection byte cap exceeded")
    require(elapsed_seconds <= WALL_CAP_SECONDS, "projection wall cap exceeded")


def write_output(output: Path, encoded: bytes) -> None:
    require(not output.exists(), "refusing to overwrite projection")
...
"""
    ...
    projected = project_content(
        {
            "enemies": {"a": {"moves": {"x": {"name": "X", "intent": "attack", "dmg": 2}}}},
            "encounters": [{"weak": [], "normal": [["a", "a"], ["a"]], "elite": [], "boss": []}],
        }
    )
    assert projected["enemyMoves"] == {"a": {"x": {"dmg": 2, "intent": "attack"}}}
    assert projected["multiEnemyGroupCount"] == 1
    try:
        clean_move({"intent": "attack", "unknown": 1})
        raise AssertionError("unknown move key accepted")
    except ValueError:
        pass
...
def build(source: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "executionAccounting": {
            "sourceFilesRead": len(SOURCE_HASHES),
            "gitSubprocesses": 2,
            "gitCommands": ["rev-parse HEAD", "status --porcelain --untracked-files=all"],
            "projectionMechanicalCorrectionsUsed": 1,
            "GodotProcesses": 0,
            "simulatorRows": 0,
            "cacheReads": 0,
            "cacheWrites": 0,
...
def main() -> None:
    ...
    enforce_caps(len(encoded), elapsed)
