#!/usr/bin/env python3
"""Zero-new-row capacity audit for the private player-status liability."""
...
PROTOCOL = core.ROOT / "protocols/post-v30-status-liability-capacity-v1.json"
SUMMARY = core.ROOT / "summaries/post-v30-status-liability-capacity-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Status-liability capacity mismatch: {label}")
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite status-liability capacity summary")
    ...
    for name, spec in protocol["priorEvidence"].items():
        ...
        require(f"{name} decision", json.loads(path.read_text())["decision"] ==
                spec["decision"])
    anchor_spec = protocol["anchorProtocol"]
    anchor_path = core.ROOT / anchor_spec["path"]
    require("anchor protocol SHA", core.file_sha(anchor_path) ==
            anchor_spec["sha256"])
    anchor_protocol = json.loads(anchor_path.read_text())
    ...
    require("baseline SHA", core.file_sha(baseline_path) ==
            protocol["baseline"]["outputSha256"])
    require("trace SHA", core.file_sha(trace_path) ==
    ...
    require("baseline plan", baseline_output["planSha256"] ==
            protocol["baseline"]["planSha256"])
    require("trace plan", trace_output["planSha256"] ==
    ...
    for key, row in rows.items():
        require("trace current identity",
    ...
    anchors = anchor_protocol["sharedAnchors"]
    require("Scoreline anchor", sorted(scoreline) ==
            anchors["scoreline"]["policies"])
    require("Afterimage anchor", sorted(afterimage) ==
            anchors["afterimageSubstrate"]["policies"])
    separations = {
        "scoreline": capacity.separation(candidate_active, scoreline),
        "afterimage": capacity.separation(candidate_active, afterimage),
    }
    baseline_faults = sum(
    ...
    gate_results = {
        ...
        "scorelineSeparation": (
            separations["scoreline"]["candidateOnlyPolicies"] >=
            gates["minimumCandidateOnlyPolicies"]
            and separations["scoreline"]["anchorOnlyPolicies"] >=
            gates["minimumAnchorOnlyPolicies"]
            and separations["scoreline"]["jaccard"] <= gates["maximumAnchorJaccard"]),
        "afterimageSeparation": (
            separations["afterimage"]["candidateOnlyPolicies"] >=
            gates["minimumCandidateOnlyPolicies"]
            and separations["afterimage"]["anchorOnlyPolicies"] >=
            gates["minimumAnchorOnlyPolicies"]
            and separations["afterimage"]["jaccard"] <= gates["maximumAnchorJaccard"]),
        "reliability": baseline_faults <= gates["maximumBaselineFaultRows"],
    }
    elapsed = time.monotonic() - started
    if elapsed > protocol["budget"]["maximumWallTimeSeconds"]:
        boundary, decision = 3, "record-status-liability-capacity-inconclusive-at-cap"
    elif all(gate_results.values()):
        boundary, decision = 1, "authorise-status-liability-factor-identity"
    else:
        boundary, decision = 2, "close-status-liability-at-capacity"
    ...
    summary = {
        ...
        "separations": separations, "policySets": policy_sets,
