#!/usr/bin/env python3
"""Zero-row private counter/tag/transform design audit for issue #421."""
...
PROTOCOL = core.ROOT / "protocols/post-843e899-private-state-design-v1.json"
SUMMARY = core.ROOT / "summaries/post-843e899-private-state-design-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Private-state design mismatch: {label}")
...
def classify(
    ...
    if not legal:
        return (
            "futility", 2,
            "close-private-counter-tag-transform-and-raise-owner-authority-package",
            [],
        )
    if len(legal) == 1 or common.rank(legal[0]) < common.rank(legal[1]):
        return "success", 1, "freeze-private-state-contract-for-preflight", legal[:1]
    return "inconclusive", 3, "record-private-state-design-inconclusive-at-cap", []
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite private-state design summary")
    ...
    require("serial dispositions", all(
        evidence_decisions[item["evidenceId"]] == item["decision"]
        for item in protocol["serialFamilyDispositions"]
    ))
    ...
    require("all legal contracts have twelve fields", all(
        len(candidate.get("contract", {})) == 12
        for candidate in candidates if common.eligible(candidate, protocol["requiredGates"])
    ))
    require("all legal contracts use frozen order", all(
        list(candidate.get("contract", {})) == protocol["twelveFieldOrder"]
        for candidate in candidates if common.eligible(candidate, protocol["requiredGates"])
    ))
    ...
    if elapsed > protocol["budget"]["maximumWallTimeSeconds"]:
        outcome, boundary, decision, selected, selected_ids = (
            "inconclusive", 3, "record-private-state-design-inconclusive-at-cap", [], [],
        )
    ...
    summary = {
        ...
        "decision": decision,
        "allFourSerialClassesExhausted": outcome == "futility",
        ...
        "serialFamilyDispositions": protocol["serialFamilyDispositions"],
        "priorEvidenceDecisions": evidence_decisions,
        "humanAuthorityPackage": protocol["humanAuthorityPackage"] if outcome == "futility" else None,
        "protocolSha256": protocol_sha,
        "runnerSha256": core.file_sha(Path(__file__)),
        "sourceHead": immutable["sourceHead"],
        "archiveHeadPreserved": immutable["archiveHead"],
