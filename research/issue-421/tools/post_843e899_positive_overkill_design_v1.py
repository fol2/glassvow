#!/usr/bin/env python3
"""Zero-row positive-overkill excess design audit for issue #421."""
...
PROTOCOL = core.ROOT / "protocols/post-843e899-positive-overkill-design-v1.json"
SUMMARY = core.ROOT / "summaries/post-843e899-positive-overkill-design-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Positive-overkill design mismatch: {label}")
...
def eligible(candidate: dict[str, Any], required: list[str]) -> bool:
    return not candidate["closureAliases"] and all(candidate["gates"][gate] for gate in required)
...
def classify(
    ...
    legal = sorted(
        (candidate for candidate in candidates if eligible(candidate, required)), key=rank,
    )
    if not legal:
        return "futility", 2, "close-positive-overkill-excess-at-zero-row-boundary", []
    if len(legal) == 1 or rank(legal[0]) < rank(legal[1]):
        return (
            "success", 1,
            "freeze-positive-overkill-facet-salvage-contract-for-preflight",
            legal[:1],
        )
    return "inconclusive", 3, "record-positive-overkill-design-inconclusive-at-cap", []
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite positive-overkill design summary")
    protocol, protocol_sha = core.load_protocol(PROTOCOL)
    started = time.monotonic()
    immutable = protocol["immutableInputs"]
    require("runner SHA", core.file_sha(Path(__file__)) == immutable["runnerSha256"])
    require("task capsule SHA",
    ...
    for assertion in protocol["sourceAssertions"]:
        ...
        if "contains" in assertion:
            require(assertion["id"], assertion["contains"] in source)
        else:
            require(assertion["id"], assertion["absent"] not in source)
    ...
    require("candidate SHA identities", all(
        candidate_sha(candidate) == candidate["canonicalSha256"] for candidate in candidates
    ))
    ...
    if elapsed > protocol["budget"]["maximumWallTimeSeconds"]:
        outcome, boundary, decision, selected, selected_ids = (
            "inconclusive", 3, "record-positive-overkill-design-inconclusive-at-cap", [], [],
        )
