#!/usr/bin/env python3
"""Zero-row observability gate for damaging-intent Shatter suppression."""
...
PROTOCOL = core.ROOT / "protocols/post-v30-shatter-action-suppression-capacity-v1.json"
SUMMARY = core.ROOT / "summaries/post-v30-shatter-action-suppression-capacity-v1.json"
TRACE_SOURCE = core.ROOT / "lantern-art-identity-source"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Shatter action-suppression capacity mismatch: {label}")
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite Shatter action-suppression capacity summary")
    ...
    require("selected relation", evidence["actionSuppression"]["selectedRelation"]["id"] ==
            protocol["selectedRelation"]["id"])
    require("eligible cache authority", evidence["lanternCapacity"]["execution"]
            ["manifests"]["capacity"]["outputSha256"] == immutable["eligibleCacheSha256"])
    ...
    decision = {
        "futility": "close-damaging-intent-shatter-suppression-on-observability-gate",
        "inconclusive": "record-damaging-intent-shatter-suppression-inconclusive-at-cap",
    }[outcome]
    ...
    summary: dict[str, Any] = {
        ...
        "selectedRelation": protocol["selectedRelation"],
