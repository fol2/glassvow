#!/usr/bin/env python3
"""Cache-correct source, null and direct gate for issue #421 Ward spend v3."""
...
PROTOCOL = core.ROOT / "protocols/post-3a234bae-ward-spend-identity-v3.json"
SUMMARY = core.ROOT / "summaries/post-3a234bae-ward-spend-identity-v3.json"
BASELINE = core.ROOT / "ward-spend-finisher-v3-baseline"
CANDIDATE = core.ROOT / "ward-spend-finisher-v3-source"
PROBE = CANDIDATE / "tools/research_421_ward_spend_probe_v3.gd"
...
def seconds_left(deadline: float) -> int:
    ...
    if remaining < 1:
        raise TimeoutError("Ward-spend v3 direct gate reached its wall-time cap")
    return remaining
...
def source_identity(protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "baselineClassCacheSha256": core.file_sha(
            BASELINE / ".godot/global_script_class_cache.cfg"),
        "candidateClassCacheSha256": core.file_sha(
            CANDIDATE / ".godot/global_script_class_cache.cfg"),
        "baselineSha256": {
...
def parse_preflight(godot: str, deadline: float) -> tuple[list[str], str]:
    ...
    result = subprocess.run(
        [str(CANDIDATE / "tools/check_scripts.sh"),
         ...
         "tools/research_421_ward_spend_probe_v3.gd"],
...
def run_probe(
    ...
    with tempfile.TemporaryDirectory(dir=core.WORK, prefix="ward-spend-v3-") as tmp:
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite Ward-spend v3 identity summary")
    ...
    source_identity_faults = [] if actual_source == expected_source else [
    ...
    source_gate_faults = (engine_preflight_faults + source_identity_faults
                          + static_faults() + seed_faults(protocol))
    ...
    if post_source != actual_source:
        faults.append("immutable source or generated class cache changed during execution")
    elapsed = time.monotonic() - started
    if execution_error or elapsed > protocol["budget"]["maximumWallTimeSeconds"]:
        outcome = "inconclusive"
        decision = "record-ward-spend-v3-identity-inconclusive-at-cap"
        boundary = 3
    elif faults:
        outcome = "futility"
        decision = "close-ward-spend-v3-representation-at-source-direct-gate"
        boundary = 2
    else:
        outcome = "success"
        decision = "authorise-brace-first-natural-capacity-screen-v3"
        boundary = 1
    summary = {
        "schemaVersion": 3, "issue": 421, "outcomeClass": outcome,
