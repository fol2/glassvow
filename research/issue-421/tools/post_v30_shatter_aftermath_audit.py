#!/usr/bin/env python3
"""Zero-row source audit for exact Shatter aftermath branches."""
...
PROTOCOL = core.ROOT / "protocols/post-v30-shatter-aftermath-audit-v1.json"
SUMMARY = core.ROOT / "summaries/post-v30-shatter-aftermath-audit-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Shatter aftermath mismatch: {label}")
...
def route_inventory(content: dict[str, Any]) -> dict[str, Any]:
    ...
    unlock_ids = sorted({
        ...
        for deed in content["deeds"] for unlock in deed.get("unlocks", [])
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite Shatter aftermath summary")
    ...
    require("route inventory", inventory == protocol["naturalRouteIdentity"])
    ...
    partition = protocol["aftermathBranchPartition"]
    require("branch order", [row["id"] for row in partition] == [
    ...
    require("one uncovered branch", selected == [protocol["selectedRelation"]["id"]])

    ledger_before = identity.ledger_identity()
    require("ledger freeze", ledger_before == protocol["ledgerFreeze"])
    elapsed = time.monotonic() - started
    if elapsed > float(protocol["budget"]["maximumWallTimeSeconds"]):
        boundary, decision = 3, "record-shatter-aftermath-inconclusive-at-cap"
    else:
        boundary, decision = 1, "freeze-shatter-smolder-transfer-for-capacity-preregistration"
    ledger_after = identity.ledger_identity()
    require("zero-row ledger identity", ledger_after == ledger_before)
    authority = "successAuthority" if boundary == 1 else "inconclusiveAuthority"
    summary = {
        "schemaVersion": 1,
        "issue": 421,
        ...
        "authority": protocol["decisionRules"][authority],
    }
