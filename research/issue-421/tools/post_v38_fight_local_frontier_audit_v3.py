...
PROTOCOL = core.ROOT / "protocols/post-v38-fight-local-frontier-audit-v3.json"
SUMMARY = core.ROOT / "summaries/post-v38-fight-local-frontier-audit-v3.json"
...
def main() -> None:
    ...
    preflight = protocol["supersedesPreExecutionFailure"]
    require(
        "v2 protocol identity",
        core.file_sha(core.ROOT / preflight["protocolPath"])
        == preflight["protocolSha256"],
    )
    require(
        "v2 runner identity",
        core.file_sha(core.ROOT / preflight["runnerPath"])
        == preflight["runnerSha256"],
    )
    require(
        "v2 preflight identity",
        core.file_sha(core.ROOT / preflight["failurePath"])
        == preflight["failureSha256"],
    )
