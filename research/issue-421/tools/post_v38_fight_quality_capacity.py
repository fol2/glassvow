#!/usr/bin/env python3
"""Zero-row inventory for exact-equality fight-quality triggers."""
...
PROTOCOL = core.ROOT / "protocols/post-v38-fight-quality-capacity-v1.json"
SUMMARY = core.ROOT / "summaries/post-v38-fight-quality-capacity-v1.json"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Fight-quality capacity mismatch: {label}")


def active_row(candidate_id: str, row: dict[str, Any]) -> bool:
    for fight in row["fights"]:
        if fight.get("result") != "win":
            continue
        if candidate_id == "no-hp-loss-victory" and int(fight["hpLost"]) == 0:
            return True
        if candidate_id == "one-turn-victory" and int(fight["turns"]) == 1:
            return True
    return False
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite the fight-quality summary")
    ...
    simulator_text = capacity.main_blob("tools/balance_sim.gd").decode()
    for fragment in protocol["sourceAssertions"]:
        require(f"simulator source {fragment}", fragment in simulator_text)
    ...
    for key, row in rows.items():
        ...
        for fight in row["fights"]:
            require("complete nonnegative fight-quality telemetry",
                    set(fight) == {"act", "kind", "enemies", "result", "turns", "hpLost",
                                   "shatters", "smolderKills"}
                    and int(fight["turns"]) >= 0 and int(fight["hpLost"]) >= 0)

    def won_fight(row: dict[str, Any]) -> bool:
        return any(fight.get("result") == "win" for fight in row["fights"])

    def afterimage_route(row: dict[str, Any]) -> bool:
        return int(row.get("packageEvents", {}).get("afterimageGuardDamage", 0)) > 0

    opportunity_policies = trace.policy_set(rows, protocol, won_fight)
    ...
    for candidate in protocol["candidates"]:
        ...
        active = lambda row, item=candidate_id: active_row(item, row)
        ...
        observed_kinds = sorted({
            str(fight["kind"]) for row in rows.values() for fight in row["fights"]
            if fight.get("result") == "win" and (
                (candidate_id == "no-hp-loss-victory" and int(fight["hpLost"]) == 0)
                or (candidate_id == "one-turn-victory" and int(fight["turns"]) == 1)
            )
        })
        ...
        counts = {
            ...
            "observedFightKinds": observed_kinds,
    ...
    if elapsed > float(protocol["budget"]["maximumWallTimeSeconds"]):
        boundary = 3
        decision = "record-fight-quality-capacity-inconclusive-at-cap"
        selected = None
    elif selected is not None:
        boundary = 1
        decision = f"freeze-{selected}-trigger-for-identity-preflight"
    else:
        boundary = 2
        decision = "close-exact-equality-fight-quality-family"
    ...
    summary = {
        ...
        "sharedOpportunityPolicies": sorted(opportunity_policies),
