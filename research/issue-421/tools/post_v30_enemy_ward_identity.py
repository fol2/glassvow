#!/usr/bin/env python3
"""Preregistered enemy-Ward Facet identity preflight for issue #421."""
...
PROTOCOL = core.ROOT / "protocols/post-v30-enemy-ward-identity-v1.json"
SUMMARY = core.ROOT / "summaries/post-v30-enemy-ward-identity-v1.json"
SOURCE = core.ROOT / "enemy-ward-transition-identity-source"
GODOT = Path("/Applications/Godot.app/Contents/MacOS/Godot")
PROBE = "res://tools/research_421_enemy_ward_identity_probe.gd"


def require(label: str, condition: bool) -> None:
    if not condition:
        raise RuntimeError(f"Enemy-Ward identity mismatch: {label}")
...
def source_identity() -> dict[str, Any]:
    return {
        ...
        "probeSha256": core.file_sha(
            SOURCE / "tools/research_421_enemy_ward_identity_probe.gd"),
        "probeUidSha256": core.file_sha(
            SOURCE / "tools/research_421_enemy_ward_identity_probe.gd.uid"),
        "runnerSha256": core.file_sha(Path(__file__)),
        "frontierProtocolSha256": core.file_sha(
            core.ROOT / "protocols/post-v30-enemy-ward-transition-audit-v1.json"),
        "frontierSummarySha256": core.file_sha(
            core.ROOT / "summaries/post-v30-enemy-ward-transition-audit-v1.json"),
...
def remaining(deadline: float) -> int:
    ...
    if seconds < 1:
        raise TimeoutError("enemy-Ward identity exceeded its wall-time ceiling")
    return seconds
...
def run_probe(
    ...
    with tempfile.TemporaryDirectory(
        dir=core.WORK, prefix="enemy-ward-identity-"
    ) as tmp:
...
def normalised_direct(row: dict[str, Any]) -> dict[str, Any]:
    value = copy.deepcopy(row)
    for key in ("id", "factor", "chipEvents"):
        value.pop(key)
    for key in ("chips", "willShatter"):
        value["preview"].pop(key)
    value["enemyAfter"].pop("chips")
    value["eventKinds"] = [kind for kind in value["eventKinds"] if kind != "chip"]
    for hit in value["hitEvents"]:
        hit.pop("facetHitFlagged")
        hit.pop("researchFacetHitFlagged")
    return value
...
def check_direct(
    ...
    seen: set[str] = set()
    rng_mismatches = mapping_mismatches = 0
    ...
    off = by_id["exact-break-factor-off"]
    on = by_id["exact-break-factor-on"]
    require("focused comparison IDs excluded",
            normalised_direct(off) == normalised_direct(on))
    require("focused preview delta", off["preview"]["chips"] == 0 and
            on["preview"]["chips"] == 1)
    require("focused state delta", off["enemyAfter"]["chips"] == 0 and
            on["enemyAfter"]["chips"] == 1)
    require("focused event delta", off["chipEvents"] == [] and
            on["chipEvents"] == [{"idx": 0, "n": 1, "chips": 1, "facetMax": 4}])
    return {
        "rows": len(rows),
...
def trace_schema_faults(trace: Any) -> int:
    ...
    for event in trace["combatEvents"]:
        ...
        if not isinstance(event, dict):
            continue
        if event.get("t") == "play":
            faults += any(key not in event for key in ("targetIdx", "id", "uid"))
        elif event.get("t") == "chip":
            faults += any(key not in event for key in ("idx", "n", "chips", "facetMax"))
        elif event.get("t") == "hitEnemy":
            keys = ("idx", "amount", "blocked", "blockBefore", "blockAfter",
                    "pendingChipsActive", "isAttack", "duskAspect",
                    "wardBreakWithoutBlood", "facetHitFlagged",
                    "researchFacetHitFlagged")
            faults += any(key not in event for key in keys)
            if all(key in event for key in keys):
                before, after = int(event["blockBefore"]), int(event["blockAfter"])
                amount, blocked = int(event["amount"]), int(event["blocked"])
                transition = before > 0 and after == 0 and amount == 0
                faults += not (before >= after >= 0 and blocked == before - after)
                faults += event["wardBreakWithoutBlood"] is not transition
                current_hit = event["pendingChipsActive"] is True \
                    and event["isAttack"] is True and amount > 0
                faults += event["researchFacetHitFlagged"] is not False
                faults += event["facetHitFlagged"] is not current_hit
    return int(faults)
...
def check_sentinel(
    ...
    complete_mismatches = schema_faults = 0
...
def check_identity(
    ...
    policy_mismatches = result_mismatches = reliability_faults = schema_faults = 0
...
def main() -> None:
    if SUMMARY.exists():
        raise RuntimeError("refusing to overwrite enemy-Ward identity summary")
    ...
    if success:
        outcome_class = "success"
    decision = {
        "success": "enemy-ward-identity-green-authorise-capacity-preregistration",
        "futility": "close-enemy-ward-transition-on-identity-failure",
        "inconclusive": "record-enemy-ward-identity-inconclusive-at-cap",
    }[outcome_class]
    summary = {
        "schemaVersion": 1,
        "issue": 421,
