extends SceneTree
## Direct source/interface/null probe for issue #421 positive-overkill excess.

const EXCESS_EVENT: String = "research421PositiveOverkill"
...
func _run_row(content: ContentDB, spec: Dictionary) -> Dictionary:
	...
	var run: RunState = RunState.new_run(
		content, _int(spec, "seed"), "positive-overkill-%s" % str(spec["id"]), profile)
	...
	for state_v: Variant in enemy_states:
		...
		enemy_ids.append(str(state.get("id", "shade")))
	...
	var factor_available: bool = game.rules.has_method(
		"configure_research421_positive_overkill")
	if configured:
		if not factor_available:
			return {"id": str(spec["id"]), "error": "excess interface unavailable"}
		game.rules.call("configure_research421_positive_overkill", producer, consumer)
	...
	var excess_events: Array[Dictionary] = []
	var ember_events: Array[Dictionary] = []
	var event_index: int = 0
	for event_v: Variant in queue:
		var event: Dictionary = event_v
		if str(event.get("t", "")) == EXCESS_EVENT:
			excess_events.append(event.duplicate(true))
		else:
	...
	return {
		...
		"factorAvailable": factor_available, "mode": mode,
		"playReturned": play_returned, "state": game.cb.to_dict(),
		"enemiesAfter": enemies_after, "queue": queue,
		"baselineEvents": baseline_events, "precisionEvents": excess_events,
...
func _fail(message: String) -> void:
	push_error("research_421_positive_overkill_probe: %s" % message)
	quit(2)
