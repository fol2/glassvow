...
static var _research421_overflow_producer: bool = false
static var _research421_overflow_consumer: bool = false
...
static func configure_research421_ember_overflow(
	producer_enabled: bool, consumer_enabled: bool
) -> void:
	_research421_overflow_producer = producer_enabled
	_research421_overflow_consumer = consumer_enabled
...
func gain_embers(run: RunState, cb: CombatState, n: int) -> int:
	...
	var lost: int = maxi(0, n - delta)
	if _research421_overflow_producer and run.aspect == 0 and lost > 0 \
			and cb.player.research421_ember_overflow == 0:
		cb.player.research421_ember_overflow = 1
		cb.queue.append({
			"t": &"research421EmberOverflow", "stage": "producer",
			"requested": n, "realised": delta, "lost": lost,
			"totalBefore": cb.embers, "cap": cb.ember_cap, "turn": cb.turn,
		})
		cb.queue.append({
			"t": &"research421EmberOverflow", "stage": "mediator-set",
			"mark": 1, "turn": cb.turn,
		})
	if delta == 0:
		return 0
...
func lose_combat(run: RunState, cb: CombatState) -> void:
	...
	cb.player.hp = 0
	run.player.hp = 0
	_expire_research421_overflow(cb, "loss")
...
func _win_combat(run: RunState, cb: CombatState) -> void:
	cb.over = true
	cb.result = "win"
	_expire_research421_overflow(cb, "victory")
...
func play_card(run: RunState, cb: CombatState, uid: int, target_idx: Variant = null) -> bool:
	...
	var research421_overflow_bonus: int = 0
	if _research421_overflow_consumer and run.aspect == 0 \
			and inst.id == &"heavyBlow" and p.research421_ember_overflow == 1:
		p.research421_ember_overflow = 0
		research421_overflow_bonus = 3
		cb.queue.append({
			"t": &"research421EmberOverflow", "stage": "consumer",
			"uid": inst.uid, "id": String(inst.id), "targetIdx": target_idx,
			"turn": cb.turn,
		})
		cb.queue.append({
			"t": &"research421EmberOverflow", "stage": "mediator-consumed",
			"mark": 0, "turn": cb.turn,
		})
		cb.queue.append({
			"t": &"research421EmberOverflow", "stage": "payoff",
			"operation": "incremental-damage", "n": research421_overflow_bonus,
	...
	for fx_v: Variant in effects:
		...
		_apply_effect(
			run, cb, inst, d, fx, target, seal_mult, research421_overflow_bonus)
		research421_overflow_bonus = 0
...
func _apply_effect(
	...
	damage_mult: int = 1,
	research421_overflow_bonus: int = 0
) -> void:
	var p: PlayerCombatant = cb.player
	...
	match kind:
		"dmg":
			...
			var n: int = _ji(fx["n"]) + research421_overflow_bonus
...
func end_turn(run: RunState, cb: CombatState) -> void:
	...
	_expire_research421_overflow(cb, "end-turn")
...
func _expire_research421_overflow(cb: CombatState, reason: String) -> void:
	if cb.player.research421_ember_overflow == 0:
		return
	cb.queue.append({
		"t": &"research421EmberOverflow", "stage": "expiry",
		"reason": reason, "mark": cb.player.research421_ember_overflow,
		"turn": cb.turn,
	})
	cb.player.research421_ember_overflow = 0
...
func preview_play(
	...
	var block: int = 0
	var research421_overflow_bonus: int = 0
	if _research421_overflow_consumer and run != null and run.aspect == 0 \
			and inst.id == &"heavyBlow" and p.research421_ember_overflow == 1:
		research421_overflow_bonus = 3
	var effects: Array = d.get("effects", [])
	for fx_v: Variant in effects:
		...
		if kind == "dmg":
			hits.append({
				"dmg": _preview_hit(p, target, _ji(fx["n"]) + research421_overflow_bonus),
