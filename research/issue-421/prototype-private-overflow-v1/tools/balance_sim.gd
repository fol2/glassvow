...
static func _harvest_fight(game: GlassvowGame) -> void:
	...
	var research421_overflow_mark: bool = false
	for event_v: Variant in game.cb.queue:
		...
		if kind == "turn":
			...
			if research421_overflow_mark and last_play == "heavyBlow":
				_bump("emberOverflowEligibleConsumers")
				_bump("emberOverflowEligibleConsumer_heavyBlow")
				research421_overflow_mark = false
		elif kind == "research421EmberOverflow":
			var stage: String = str(event.get("stage", ""))
			_bump("emberOverflow%s" % stage.to_pascal_case())
			if stage == "producer":
				_bump("emberOverflowRequested", _ji(event.get("requested", 0)))
				_bump("emberOverflowLost", _ji(event.get("lost", 0)))
			elif stage == "mediator-set":
				if research421_overflow_mark:
					_bump("emberOverflowMediatorSetWhileMarked")
				research421_overflow_mark = true
			elif stage == "expiry":
				var reason: String = str(event.get("reason", ""))
				_bump("emberOverflowExpiry%s" % reason.to_pascal_case())
				if research421_overflow_mark:
					_bump("emberOverflowUnavailableExpiries")
					research421_overflow_mark = false
				else:
					_bump("emberOverflowExpiryAfterEligible")
		elif kind == "status":
	...
	if research421_overflow_mark:
		_bump("emberOverflowFinalOpenMarks")
