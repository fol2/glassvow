extends SceneTree
## Fixed-policy whole-run capacity probe for issue #421 Ember overflow.
...
func _initialize() -> void:
	...
	var factor_available: bool = rules.has_method(
	...
	for spec_v: Variant in rows_v:
		...
		if configured:
			if not factor_available:
				_fail("overflow interface unavailable")
				return
			rules.call("configure_research421_ember_overflow", producer, consumer)
...
func _fail(message: String) -> void:
	push_error("research_421_ember_overflow_capacity_probe: %s" % message)
	quit(2)
