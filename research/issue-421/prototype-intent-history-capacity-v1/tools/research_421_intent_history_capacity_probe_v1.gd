extends SceneTree
## Fixed-policy whole-run capacity probe for issue #421 intent history.
...
func _initialize() -> void:
	...
	var factor_available: bool = rules.has_method("configure_research421_intent_history")
	...
	for spec_v: Variant in rows_v:
		...
		if configured:
			if not factor_available:
				_fail("intent-history interface unavailable")
				return
			rules.call("configure_research421_intent_history", producer, consumer)
...
func _fail(message: String) -> void:
	push_error("research_421_intent_history_capacity_probe: %s" % message)
	quit(2)
