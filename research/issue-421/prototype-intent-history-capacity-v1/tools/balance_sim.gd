...
static func _record_intent_history_response(pending: Dictionary, eligible: bool) -> void:
	if eligible:
		_bump("intentHistoryEligibleCards")
		_bump("intentHistoryEligibleCard_%s" % str(pending.get("card", "")))
		_bump("intentHistoryEligibleMove_%s" % str(pending.get("move", "")))
	else:
		_bump("intentHistoryFinalResponses")
...
static func _harvest_fight(game: GlassvowGame) -> void:
	...
	var last_play_uid: int = -1
	var intent_pending: Dictionary = {}
	for event_v: Variant in game.cb.queue:
		...
		if kind == "turn":
			...
			last_play_uid = _ji(event.get("uid", -1))
			_bump("%sPlayed" % last_play)
		elif kind == "research421IntentHistory":
			var stage: String = str(event.get("stage", ""))
			_bump("intentHistory%s" % stage.to_pascal_case())
			if stage == "ai-boundary" and event.get("researchKeyPresent", false) == true:
				_bump("intentHistoryAiBoundaryKeyFault")
			elif stage == "expiry":
				var reason: String = str(event.get("reason", ""))
				_bump("intentHistoryExpiry%s" % reason.to_pascal_case())
				if reason == "consumer-disabled":
					if not intent_pending.is_empty():
						_bump("intentHistoryTelemetryFault")
					intent_pending = {
						"card": last_play, "uid": last_play_uid,
						"move": str(event.get("move", "")),
					}
			elif stage == "payoff":
				_bump("intentHistoryRequested", _ji(event.get("requested", 0)))
				_bump("intentHistoryRealised", _ji(event.get("realised", 0)))
		elif kind in ["victory", "defeat"] and not intent_pending.is_empty():
			_record_intent_history_response(intent_pending, false)
			intent_pending = {}
		elif kind in ["toDiscard", "exhaust"] and not intent_pending.is_empty() \
				and _ji(event.get("uid", -1)) == _ji(intent_pending.get("uid", -2)):
			_record_intent_history_response(intent_pending, true)
			intent_pending = {}
		elif kind == "status":
	...
	for enemy: EnemyCombatant in game.cb.enemies:
		if enemy.flags.has("_research421IntentHistory"):
			_bump("intentHistoryResidualMarkers")
