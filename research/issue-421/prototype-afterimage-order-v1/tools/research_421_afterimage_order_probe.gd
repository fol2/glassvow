extends SceneTree
## Deterministic identity probe for issue #421 Afterimage order control.
...
func _scripted(content: ContentDB, spec: Dictionary) -> Dictionary:
	...
	if typeof(settings_v) != TYPE_DICTIONARY:
		return {"error": "research421 must be a dictionary"}
	var settings: Dictionary = settings_v
	var research_fault: String = Sim.configure_research421_fight_local(settings)
	...
	for action_v: Variant in spec.get("actions", []):
		...
		elif kind == "setAfterimageWard":
			game.cb.research421_afterimage_ward = int(float(str(action["value"])))
	...
	return {
		...
		"research421": {
			...
			"afterimageWard": game.cb.research421_afterimage_ward,
		},
	}
...
func _choice_snapshot(game: GlassvowGame) -> Dictionary:
	...
	return {
		...
		"research421AfterimageOrder": pick.get("research421AfterimageOrder", false),
...
func _fail(message: String) -> void:
	push_error("research_421_afterimage_order_probe: %s" % message)
	quit(2)
