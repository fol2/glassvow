...
const RESEARCH421_SCORELINE_LEVELS: Dictionary = {
	"off": 0, "chisel-base-4": 4, "faultline-bonus-6": 6, "faultline-base-8": 8,
}
const RESEARCH421_AFTERIMAGE_LEVELS: Dictionary = {
	"off": 0, "wardens-edge-4": 4, "ward-base-5": 5, "ward-upgraded-8": 8,
}
const RESEARCH421_AFTERIMAGE_ORDER_LEVELS: Dictionary = {
	"current": false, "ward-before-edge": true,
}
...
static func simulate(content: ContentDB, aspect: String, seed: int, vow: int = 0,
	...
	_probe = {}
	var research_fault: String = configure_research421_fight_local(research421)
...
static func configure_research421_fight_local(settings: Dictionary) -> String:
	CombatRules.set_research421_fight_local(0, 0)
	Pilot.set_research421_afterimage_order(false)
	if settings.is_empty():
		return ""
	if not settings.has("schemaVersion") or typeof(settings["schemaVersion"]) != TYPE_STRING \
			or str(settings["schemaVersion"]) != "fight-local-order-v1":
		return "research421 requires schemaVersion=fight-local-order-v1"
	for key_v: Variant in settings.keys():
		var key: String = str(key_v)
		if key not in [
			"schemaVersion", "scorelinePayoff", "afterimagePayoff", "afterimageOrder",
		]:
			return "research421 unknown key %s" % key
	var scoreline_v: Variant = settings.get("scorelinePayoff", "off")
	if typeof(scoreline_v) != TYPE_STRING \
			or not RESEARCH421_SCORELINE_LEVELS.has(str(scoreline_v)):
		return "research421 scorelinePayoff has unregistered level"
	var afterimage_v: Variant = settings.get("afterimagePayoff", "off")
	if typeof(afterimage_v) != TYPE_STRING \
			or not RESEARCH421_AFTERIMAGE_LEVELS.has(str(afterimage_v)):
		return "research421 afterimagePayoff has unregistered level"
	var order_v: Variant = settings.get("afterimageOrder", "current")
	if typeof(order_v) != TYPE_STRING \
			or not RESEARCH421_AFTERIMAGE_ORDER_LEVELS.has(str(order_v)):
		return "research421 afterimageOrder has unregistered level"
	if str(order_v) == "ward-before-edge" and str(afterimage_v) == "off":
		return "research421 ward-before-edge requires Afterimage payoff"
	var scoreline: int = _ji(RESEARCH421_SCORELINE_LEVELS[str(scoreline_v)])
	var afterimage: int = _ji(RESEARCH421_AFTERIMAGE_LEVELS[str(afterimage_v)])
	CombatRules.set_research421_fight_local(scoreline, afterimage)
	Pilot.set_research421_afterimage_order(
		RESEARCH421_AFTERIMAGE_ORDER_LEVELS[str(order_v)] == true
	)
	return ""
...
static func _harvest_fight(game: GlassvowGame) -> void:
	...
	for event_v: Variant in game.cb.queue:
		...
		if kind == "turn":
			...
			var scoreline_stage: String = str(event.get("stage", ""))
			_bump("scoreline%sEvents" % scoreline_stage.to_pascal_case())
			if scoreline_stage == "payoff":
				_bump("scorelinePayoffRequested", _ji(event.get("requested", 0)))
				_bump("scorelinePayoffRealised", _ji(event.get("realised", 0)))
			elif scoreline_stage == "expiry":
				_bump("scorelineExpiry%s" % str(event.get("reason", "")).to_pascal_case())
		elif kind == "research421Afterimage":
			var afterimage_stage: String = str(event.get("stage", ""))
			_bump("afterimage%sEvents" % afterimage_stage.to_pascal_case())
			if afterimage_stage == "producer":
				_bump("afterimageProducerWard", _ji(event.get("realisedWard", 0)))
				_bump("afterimageStoredWard", _ji(event.get("storedWard", 0)))
			elif afterimage_stage == "payoff":
				_bump("afterimagePayoffRequested", _ji(event.get("requested", 0)))
				_bump("afterimagePayoffRealised", _ji(event.get("realised", 0)))
			elif afterimage_stage == "expiry":
				_bump("afterimageExpiry%s" % str(event.get("reason", "")).to_pascal_case())
		elif kind == "research421AfterimageOrder":
			_bump("afterimageOrderInterventionEvents")
		elif kind == "relicProc":
