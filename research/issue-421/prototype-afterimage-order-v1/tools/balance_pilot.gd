...
static var research421_afterimage_order: bool = false
static func set_modes(build: bool, play: bool) -> void:
	random_build = build
	random_play = play
static func set_research421_afterimage_order(enabled: bool) -> void:
	research421_afterimage_order = enabled
...
static func _play_cards(game: GlassvowGame) -> void:
	...
	for _guard: int in range(32):
		...
		if pick.is_empty():
			return
		var research_order: bool = pick.get("research421AfterimageOrder", false) == true
		game.apply({"t": "playCard", "uid": pick["uid"], "target": pick["target"]})
		if game.last_ret != true:
			return
		if research_order:
			game.cb.queue.append({
				"t": &"research421AfterimageOrder", "stage": "intervention",
			})
		if game.cb.over:
			return
static func _pick_play(game: GlassvowGame) -> Dictionary:
	...
	var research_pick: Dictionary = _research421_afterimage_pick(game)
	if not research_pick.is_empty():
		return research_pick
...
static func _research421_afterimage_pick(game: GlassvowGame) -> Dictionary:
	if not research421_afterimage_order or game.run.aspect != 0 \
			or game.cb.research421_afterimage_ward > 0:
		return {}
	var defend: CardInst = null
	var guarded: CardInst = null
	for card: CardInst in game.cb.hand:
		if card.id == &"defend" and defend == null:
			defend = card
		elif card.id == &"guardedStrike" and guarded == null:
			guarded = card
	if defend == null or guarded == null:
		return {}
	var guarded_data: Dictionary = game.rules.card_data(guarded)
	var guarded_target: Variant = _target(game, guarded, guarded_data)
	if not game.rules.can_play(game.run, game.cb, defend, null) \
			or not game.rules.can_play(game.run, game.cb, guarded, guarded_target):
		return {}
	var sequence_cost: int = game.rules.eff_cost(game.run, game.cb, defend) \
			+ int(float(str(guarded_data.get("cost", 0))))
	if sequence_cost > game.cb.player.energy:
		return {}
	return {"uid": defend.uid, "target": null, "research421AfterimageOrder": true}
