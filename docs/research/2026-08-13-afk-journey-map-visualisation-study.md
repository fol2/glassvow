...
7. **Zero map art ships today.** `assets/art/map/` does not exist, so
   `MapStrip.fetch` returns null and every band renders its procedural fallback.
...
|---|---|---|---|---|---|---|---|
| 1 | **Painterly colour-block texture** | 色块 structure, spot texture, soft brushstrokes ([17173](https://news.17173.com/content/08132024/101501449.shtml), [GameRes 910615](https://www.gameres.com/910615.html)) | Authored raster only. `Texture2D` via `MapStrip.fetch` → `MapStrip.draw_tiled` (`CanvasItem.draw_texture_rect`). Import **VRAM Compressed** (ASTC 4×4), never Lossy | **EXISTS** — `presentation/map/map_strip.gd` (`fetch`) → (`draw_tiled`); called at `presentation/map/map_band.gd` (`apply_region`) on both SkyBand and RegionBand. Drop PNGs into `assets/art/map/` (**directory does not exist today**) | 1 full-screen blend per painted band; sky band can be authored opaque → free of blend | 5.06 MiB/layer at 3072×1728 ASTC 4×4; +33% with mips (*estimate*) | Art-production risk, not engine risk. #87 records two generation rounds that produced synthesis, not art |
...
| 3 | **Bright desaturated stage in dark saturated surround** | Measured V=0.68/S=0.15 clearing against V=0.35/S=0.38–0.52 canopy ([App Store](https://apps.apple.com/us/app/afk-journey/id1628970855)) | Composition rule expressed in the same palette table + the existing ground scrim `draw_rect` and marker glow | **EXISTS** — path-band bed/verge draws and marker glow in `presentation/map/map_band.gd` (`_draw_bed`), (`_draw_ramp`) | Zero | Zero | Low. Conflicts with the current "everything is dark night-glass" grade — this is the change that most alters the map's felt identity |
| 4 | **Aerial perspective by saturation loss + hue cooling at constant value** | Measured far S=0.38 h193° vs near S=0.52 h205°, both V=0.35 | `shader_type canvas_item; render_mode unshaded;` mix `COLOR.rgb` toward a fog tint by a per-band `uniform float depth`. Zero extra passes | **NO SHADER PATH YET** — no `ShaderMaterial` exists anywhere under `presentation/map/`, but a `MapBand` is a `Control` i.e. a `CanvasItem`, so `material` works. Precedent: `presentation/combat/combat_screen.gd`. **Missing:** nothing pushes uniforms — `presentation/map/map_band.gd` (`set_view`) carries only cam-x and drift, and the depth projection `presentation/map/world_map_screen.gd` (`depth_of`) is GDScript-only | ~3–5 ALU per pixel per band, no new layers, no new blends | Zero | Low. Uniform plumbing through `set_view` is the whole job |
| 5 | **Height fog / distance fog / cloud projection** | Height fog + distance fog + per-region 云投影 「为场景营造空间感」 ([GameRes 905985](https://www.gameres.com/905985.html)) | Same shader as #4 with a vertical gradient term; cloud projection as one slow-scrolling multiply strip or a shader `sin`-warp on the region band | Fog disc already exists on the sky band (`SkyField.disc()` inside `presentation/map/map_band.gd` `_draw`); a cloud-projection strip is a **new** `MapStrip` kind | Fog term: free (same pass as #4). A separate cloud strip: +1 full-screen blend — counts against the ≤4 budget | Fog: zero. Cloud strip: 5.06 MiB/act (*estimate*) | Medium — a scrolling multiply strip is exactly the "visible repetition" defect measured on #87 round 1 (motif recurring every ~300 px on a 606 px draw) |
...
| 7 | **Region-scripted weather** | Per-region weather shifting light colour, lowering light/shadow contrast, changing vegetation sway frequency and amplitude, 随机频闪+雷声 ([GameRes 905985](https://www.gameres.com/905985.html)) | Already the design. `CPUParticles2D`/`GPUParticles2D` would be the idiomatic upgrade; note `emit_particle()` is Forward+/Mobile only, and `amount_ratio` gives **no** perf benefit ([GPUParticles2D docs](https://github.com/godotengine/godot-docs/blob/master/doc/classes/GPUParticles2D.xml)) | **EXISTS** — `presentation/map/map_band.gd` VeilBand (`_draw`) draws 128 disc quads / storm streaks; lightning flash via (`set_flash`) — its screen driver `_step_lightning` retired with the sky/region bands in #234 slice 7b1; act 2's lightning successor (3D environment pulse or overlay) is a 7b2 decision | Veil is already the most expensive band (parallax 1.35, ungated, 128 quads). *Estimate:* fine on SE 2, needs measuring on iPad 8 | Zero | Low — this ingredient is the one glassvow already matches |
| 8 | **Shape language (flat wedges, stacked slabs, no small noise)** | Enforced by shared authoring tools ([GameRes 905985](https://www.gameres.com/905985.html)) | `draw_polygon` / `Polygon2D` / `MeshInstance2D` (faster than a `Sprite2D` with large transparent areas) | **EXISTS** — act-0 tree polygons and sunken shafts in `presentation/map/map_band.gd` (`_draw_shafts`) and RegionBand (`_draw`) | Vertex work, negligible | Zero | Low. This is where the procedural map is already closest to the anchor |
| 9 | **Singular landmark composition** | Giant stone sword etc. as single high-value silhouettes against a saturated bed | An untiled, world-anchored `draw_texture_rect` — a `MapStrip.draw_single` sibling to `draw_tiled`, plus a placement contract | **NO PATH EXISTS — this is issue [#85](https://github.com/fol2/glassvow/issues/85) verbatim.** Every region-plane draw either tiles or wraps with `fposmod`. The Spire is *screen*-anchored on the sky band (`presentation/map/map_band.gd` (`_draw_spire`)); the rose window is world-anchored but at parallax **1.0** on the path band, and `_draw_rose_window`'s own doc block forbids painted architecture inheriting that placement | 1 small blended quad; not full-screen | ~2.25 MiB/act at 1536×1536 ASTC (*estimate*) | Medium. Also needs `clip_contents` behaviour that no band currently has — `FAR_BLEED = 8.0` depends on bands being unclipped |
...
with child order as paint order and `FAR_BLEED` overdraw depending on unclipped
bands. Adopting `Parallax2D` is a rewrite of a working system for no new
capability. *Recommendation: keep the hand-rolled band parallax.*
...
1. Push uniforms through `presentation/map/map_band.gd` (`set_view`), which today
   carries only cam-x and drift; feed it from
   `presentation/map/world_map_screen.gd` (`depth_of`).
...
shared canvas (`presentation/map/map_strip.gd` ART CONTRACT: sky ≈ 37% of stage
height, region ≈ 65% plus crown bleed):
...
2. **Activates [#85](https://github.com/fol2/glassvow/issues/85)** — the singular
   region-plane draw path. Concretely: a `MapStrip.draw_single` sibling to
   (`draw_tiled`), a world-anchored placement contract at parallax 0.35, and the
   `clip_contents` decision that `FAR_BLEED = 8.0` currently depends on being
   unset.
