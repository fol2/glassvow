...
- **H64 (`cards.devour` heal-on-kill 4→8 / 6→12, keep 10/13
  dmg, 3/4 embers, cost 1 exhaust) is noise — reverted.**
  Snapshot: `docs/balance/data/421-h64/phase-a.json`. Arm 2
  **21.0 / 8.0 / 18.5 / 6.5**. Holdout 55.0 / 22.0 / 70.5 /
  39.5 (bit-identical). Ash−Dusk **+15.5 / +17.5 pp**. Ash V0
  arm 1 77.5→77.5 (+0.0). Digest unchanged. Rare+exhaust did
  not fire n=200 planned (H50/H57/H59 pattern). Not
  relic-stack, not Dusk chip, not lantern arts, not H55
  smolder-top, not shared Dimmed, not 1-cost Dimmed/ward/
  regen, not 2-cost common smash. H39 landscape already FAIL
  C1–C3 (`595377b`); this is not a second 5h. Working SHA
  remains H39 `a0d608a5…`. Do not landscape. Do not raise
  Ash HP. Relic stack still later-phase. Readout:
...
- **H66 (`cards.twinFangs` 4→6 ×2 / 6→8 ×2, keep cost 1) is
  reverted — paid Dusk planned and random.** Snapshot:
  `docs/balance/data/421-h66/phase-a.json`. Arm 2
  **33.5 / 8.0 / 15.5 / 6.5**. Holdout 56.0 / 25.5 / 66.5 /
  39.5. Ash−Dusk **+10.5 / +14.0 pp**. Dusk V0 arm 2
  21.5→33.5 (**+12.0** C2 wrong way). Dusk V0 arm 1 **+7.0**.
  Ash V0 arm 1 +2.0 and holdout −4.0. Choir skipped (H34
  Phase A-invisible; `ashSermon` is the coal lock). Digest
  moved (`7041bfc0…`); not re-pinned. Do not buff Twin
  Shards / 1-cost shared multi-hit this loop (H53/H58 now
  live). Not relic-stack, not Dusk chip, not lantern arts,
  not H55 smolder-top, not shared Dimmed, not 1-cost
  Dimmed/ward/regen, not 2-cost common smash, not
  rare-exhaust, not First Spark draw. H39 landscape already
  FAIL C1–C3 (`595377b`); this is not a second 5h. Working
  SHA remains H39 `a0d608a5…`. Do not landscape. Do not
  raise Ash HP. Relic stack still later-phase. Readout:
...
- **H72 (`cards.frenzy` self-Cracked 2→1 / upgrade 1→0, keep
  +1 energy, cost 2) is reverted — paid Dusk planned.**
  Snapshot: `docs/balance/data/421-h72/phase-a.json`. Arm 2
  **22.0 / 7.5 / 20.5 / 7.5**. Holdout 67.0 / 28.5 / 74.0 /
  43.0. Ash−Dusk **+7.0 / +14.5 pp**. Dusk V0 holdout
  55.0→67.0 (**+12.0**). Dusk V0 arm 1 **+7.5**. Ash V0 arm 1
  +0.5 and holdout +3.5. Digest moved (`d6617bde…`); not
  re-pinned. Do not buff Overglow / shared energy rares this
  loop (H21 family). Not Emberfang n, not Emberbite damage,
  not relic-stack, not 1-pt commons, not Ashbite Ward, not
  Twin Shards, not Inner Blaze, not First Spark, not Choir.
  H39 landscape already FAIL C1–C3 (`595377b`); this is not
  a second 5h. Working SHA remains H39 `a0d608a5…`. Do not
  landscape. Do not raise Ash HP. Relic stack still
  later-phase. Readout:
  [`2026-08-21-421-hypothesis-72.md`](2026-08-21-421-hypothesis-72.md).
