...
terms run in `BODY_SHADER` (`enemy_view.gd:542-562` (`IDLE_PROFILES`),
`enemy_view.gd:2688-2692` (`_read_idle`), `enemy_view.gd:862-1187` (`BODY_SHADER`)).
...
tweened onto it (`enemy_view.gd:589` (`KIND_IDLE`); applied at
...
numbers (`enemy_view.gd:267-302` (`HEAVY_KINDS`), `enemy_view.gd:2856-2879` (`lunge`)). The hero swings once
...
indirect paths (`enemy_view.gd:3084-3093` (`_white_beat`),
...
0.6 modulate / 0.36s (`enemy_view.gd:3006-3017` (`stagger`)). `_die` awaits it before
...
legacy discs are off (`enemy_view.gd:4906-4914` (`set_doomed`),
`enemy_view.gd:862-1187` (`BODY_SHADER`)). The current fracture seams therefore never receive
...
0.32s boss (`combat_screen.gd:2244-2247`, `enemy_view.gd:3636-3667` (`mark_dead`)) — then real
...
(`enemy_view.gd:2292` (`_update_shadow`)). **Built out 2026-07-27** — the grade
...
`set_ward_shell` at `enemy_view.gd:2581-2617` (`set_ward_shell`)) — ordered facets, no hash, breaks on
...
`enemy_view.gd:2620-2681` (`_step_ward`)). Full design reasoning is in
...
from `set_facets(..., pop)` (`enemy_view.gd:4772-4779` (`set_facets`)). HP-preview `pvPulse` is
...
(`enemy_view.gd:4207-4215` (`set_targetable`)). All-target cards light every living foe, but an
...
`enemy_view.gd:3636-3667` (`mark_dead`), `combat_screen.gd:2193-2195`). Chrome matches the
...
key light (`enemy_view.gd:2110` (`_read_contact`), `enemy_view.gd:2292`
(`_update_shadow`)). **The ninth, `dy`, is now read** (2026-07-27,
`enemy_view.gd:2716` (`_read_hover`)) — this entry previously called the whole
