...
`enemy_view.gd:2420-2424` (`_read_idle`), `enemy_view.gd:816-1141` (`BODY_SHADER`)).
...
numbers (`enemy_view.gd:258-293` (`HEAVY_KINDS`), `enemy_view.gd:2558-2581` (`lunge`)). The hero swings once
...
indirect paths (`enemy_view.gd:2758-2767` (`_white_beat`),
...
0.6 modulate / 0.36s (`enemy_view.gd:2693-2704` (`stagger`)). `_die` awaits it before
...
legacy discs are off (`enemy_view.gd:4487-4495` (`set_doomed`),
`enemy_view.gd:816-1141` (`BODY_SHADER`)). The current fracture seams therefore never receive
...
0.32s boss (`combat_screen.gd:2244-2247`, `enemy_view.gd:3301-3332` (`mark_dead`)) — then real
...
`set_ward_shell` at `enemy_view.gd:2313-2350` (`set_ward_shell`)) — ordered facets, no hash, breaks on
...
`enemy_view.gd:2352-2413` (`_step_ward`)). Full design reasoning is in
...
from `set_facets(..., pop)` (`enemy_view.gd:4353-4360` (`set_facets`)). HP-preview `pvPulse` is
...
(`enemy_view.gd:3872-3880` (`set_targetable`)). All-target cards light every living foe, but an
...
`enemy_view.gd:3301-3332` (`mark_dead`), `combat_screen.gd:2193-2195`). Chrome matches the
