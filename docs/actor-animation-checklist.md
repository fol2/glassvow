...
`_play_entrance` owns the seat delay and the re-anchor (`enemy_view.gd:3018` (`enter`),
`combat_screen.gd:1502` (`_play_entrance`)). The `view.enter(...)` call in `_build_battlefield`,
...
`enemy_view.gd:2802-2806` (`_read_idle`), `enemy_view.gd:944-1269` (`BODY_SHADER`)).
...
tweened onto it (`enemy_view.gd:636` (`KIND_IDLE`); applied at
`presentation/combat/enemy_view.gd` (`_process`)):
...
an idle no fight ever runs (`presentation/lab/enemy_lab.gd` (`_actor`)). And there was no way to look at
the kind layer at all, which is the likelier reason nobody noticed: `--idle` is
now a strip mode (`presentation/lab/enemy_lab.gd` (`_ready`)). It runs in REAL time, unlike every other
...
numbers (`enemy_view.gd:294-329` (`HEAVY_KINDS`), `enemy_view.gd:2970-2993` (`lunge`)). The hero swings once
...
`cubic-bezier(.34,1.56,.64,1)` (`presentation/combat/enemy_view.gd` (`lunge`)).
...
longer flash (`enemy_view.gd:3173` (`take_hit`)).
...
keyframe nudge (`enemy_view.gd:3173` (`take_hit`)).
...
0.6 modulate / 0.36s (`enemy_view.gd:3120-3131` (`stagger`)). `_die` awaits it before
...
`cubic-bezier(.22,1,.36,1)` (`presentation/combat/enemy_view.gd` (`stagger`)).
...
The crack blaze itself is a **DECLINE** (re-judged 2026-07-31): the rule is
dead in the benchmark's shipping configuration. `.enemy.doomed .cracks path {
stroke: #ffffff }` styles the accrued-damage overlay, but `COMBAT_CRACKS =
false` (`src/ui/combat.js:2004`) makes `syncCracks` return before it draws a
path, so `.cracks` is EMPTY for the whole fight including the 820ms doom beat;
the only writer left is `igniteVessel`'s drawn fallback, which runs `warp off`
only (`src/ui/combat.js:2029-2031`) — and ignition is AFTER the beat besides.
Nothing white ever renders. The port's own seams (`crack_on`) likewise only
exist from `mark_dead`, so a `doomed` uniform would light nothing: the same
dead-rule shape as `ring()`/`slashArc()`. If a blaze is ever wanted here it is
an original design decision (build the field 820ms early), not a parity item.
...
0.32s boss (`combat_screen.gd:2244-2247`, `enemy_view.gd:3753-3784` (`mark_dead`)) — then real
...
(`presentation/combat/enemy_view.gd` (`shatter`)).
...
reference's drawn-crack cap (`enemy_view.gd:114` (`MAX_SITES`)).
...
(`enemy_view.gd:2380` (`_update_shadow`)). **Built out 2026-07-27** — the grade
...
`enemy_view.gd:45-46` (`GHOST_HOLD`), `presentation/combat/enemy_view.gd` (`_build_chrome`)). The live fill is still assigned
outright (`presentation/combat/enemy_view.gd` (`_build_chrome`)). The ghost timings match, but its
...
`set_ward_shell` at `enemy_view.gd:2679-2715` (`set_ward_shell`)) — ordered facets, no hash, breaks on
...
`enemy_view.gd:2721` (`_step_ward`)). Full design reasoning is in
...
1.3-scale / 22px-glow peak and home (`enemy_view.gd:4934` (`_block_pulse`)).
...
(`src/ui/drain.js:279-289`, `src/styles.css:955-960` / facet-row at `src/styles.css:1058`).
...
from `set_facets(..., pop)` (`enemy_view.gd:4968-4975` (`set_facets`)). HP-preview `pvPulse` is
...
`Motion.css_keyframe` with `CSS_EASE_OUT`, the same treatment as `blockPulse`.
The pips also gained the stylesheet's `.2s` state crossfade the same day
(census #988).
...
(`enemy_view.gd:4333` (`set_targetable`)). An armed single-target card now
...
from `combat_screen._float` (`combat_screen.gd:2223-2228` (`_float`)). The benchmark's crit
...
(`vfx_layer.gd:591` (`archetype_hit`), `combat_screen.gd:2619` (`_hit_enemy`)). `DEAD_KINDS_RENDER = false` drops
...
`enemy_view.gd:3753-3784` (`mark_dead`), `combat_screen.gd:2193-2195`). Chrome matches the
...
(`combat_screen.gd:1580` (`_hero_look`)); the hard-code survives only as the
...
key light (`enemy_view.gd:2198` (`_read_contact`), `enemy_view.gd:2380`
(`_update_shadow`)). **The ninth, `dy`, is now read** (2026-07-27,
`enemy_view.gd:2830` (`_read_hover`)) — this entry previously called the whole
