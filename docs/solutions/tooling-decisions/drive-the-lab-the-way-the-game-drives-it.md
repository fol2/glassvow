...
(`presentation/combat/enemy_view.gd:2613` (`_read_idle`)), and
`_resolve_profile` falls back to
`IDLE_PROFILES[&"humanoid"]` for any kind it does not know
(`presentation/combat/enemy_view.gd:2656` (`_resolve_profile`)). So every
...
`presentation/combat/enemy_view.gd:2678` (`_sync_motes`).
...
light (`presentation/combat/enemy_view.gd:2217` (`_update_shadow`)) — a derived
replacement for the
...
(`presentation/combat/enemy_view.gd:2155` (`_build_shadow`)), from the
...
(`presentation/combat/enemy_view.gd:2656` (`_resolve_profile`)), and
...
(`presentation/combat/enemy_view.gd:2613` (`_read_idle`)), so `gloomslime` —
`art.kind == "slime"`,
...
The scan was real
(`presentation/combat/enemy_view.gd:2035` (`_read_contact`)), the projection
was real (`presentation/combat/enemy_view.gd:2217` (`_update_shadow`)), and the
