...
(`presentation/combat/enemy_view.gd:2618` (`_read_idle`)), and
`_resolve_profile` falls back to
`IDLE_PROFILES[&"humanoid"]` for any kind it does not know
(`presentation/combat/enemy_view.gd:2661` (`_resolve_profile`)). So every
...
`presentation/combat/enemy_view.gd:2683` (`_sync_motes`).
...
light (`presentation/combat/enemy_view.gd:2222` (`_update_shadow`)) — a derived
replacement for the
...
(`presentation/combat/enemy_view.gd:2160` (`_build_shadow`)), from the
...
call — `set_profile`
(`presentation/combat/enemy_view.gd:2656` (`set_profile`)) — and the sheet's
...
(`presentation/combat/enemy_view.gd:2661` (`_resolve_profile`)), and
...
`presentation/combat/enemy_view.gd:2811` (`enter`) owns the motion and the fill,
...
(`presentation/combat/enemy_view.gd:2618` (`_read_idle`)), so `gloomslime` —
`art.kind == "slime"`,
...
The scan was real
(`presentation/combat/enemy_view.gd:2040` (`_read_contact`)), the projection
was real (`presentation/combat/enemy_view.gd:2222` (`_update_shadow`)), and the
