...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:1245` (`_update_shadow`); the swing itself
   enters at `enemy_view.gd:2562` (`set_light_angle`)). No amount of tuning the CSS version
...
| `--foot-ox`, `--foot-oy` | Scanned off the painting's own alpha: lowest opaque row is the contact point, its horizontal centroid is where weight sits (`enemy_view.gd:1190` (`_read_contact`)) |
...
(`enemy_view.gd:1036` (in `_build_stage`)) gives a horizontal run of roughly 1.6 body heights. That is
...
(`enemy_view.gd:1181-1182` (`CAST_MIN`)):
...
# RIGHT — fold the scale into the same basis (enemy_view.gd:1261-1265 (in _update_shadow))
