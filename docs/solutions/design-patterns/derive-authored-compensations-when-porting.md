...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:1288` (`_update_shadow`); the swing itself
   enters at `enemy_view.gd:2662` (`set_light_angle`)). No amount of tuning the CSS version
...
| `--foot-ox`, `--foot-oy` | Scanned off the painting's own alpha: lowest opaque row is the contact point, its horizontal centroid is where weight sits (`enemy_view.gd:1233` (`_read_contact`)) |
...
(`enemy_view.gd:1224-1225` (`CAST_MIN`)):
...
# RIGHT — fold the scale into the same basis (enemy_view.gd:1303-1308 (in _update_shadow))
