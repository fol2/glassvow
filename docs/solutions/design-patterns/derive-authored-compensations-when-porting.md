...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:2222` (`_update_shadow`); the swing itself
   enters at `presentation/combat/enemy_view.gd:4054` (`set_light_angle`)). No amount of tuning the CSS version
...
(`presentation/combat/enemy_view.gd:2222` (`_update_shadow`)), so it buys a shadow that
...
(`presentation/combat/enemy_view.gd:2031-2032` (`CAST_MIN`)):
...
- `assets/art/enemies/char-meta.json` — the ported per-character table. All of its
  ...
  data; `dy` is read as a resting height (`presentation/combat/enemy_view.gd:2646` (`_read_hover`)).
