...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:1417` (`_update_shadow`); the swing itself
   enters at `enemy_view.gd:2893` (`set_light_angle`)). No amount of tuning the CSS version
...
| `--foot-ox`, `--foot-oy` | Scanned off the painting's own alpha: lowest opaque row is the contact point, its horizontal centroid is where weight sits (`enemy_view.gd:1362` (`_read_contact`)) |
...
(`enemy_view.gd:1167` (in `_build_stage`)) gives a horizontal run of roughly 1.6 body heights. That is
...
(`enemy_view.gd:1353-1354` (`CAST_MIN`)):
...
# RIGHT — fold the scale into the same basis (enemy_view.gd:1429-1436 (in _update_shadow))
