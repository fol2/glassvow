...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:1852` (`_update_shadow`); the swing itself
   enters at `enemy_view.gd:3550` (`set_light_angle`)). No amount of tuning the CSS version
...
| `--foot-ox`, `--foot-oy` | Scanned off the painting's own alpha: lowest opaque row is the contact point, its horizontal centroid is where weight sits (`enemy_view.gd:1797` (`_read_contact`)) |
...
(`enemy_view.gd:1602` (in `_build_stage`)) gives a horizontal run of roughly 1.6 body heights. That is
...
(`enemy_view.gd:1788-1789` (`CAST_MIN`)):
...
# RIGHT — fold the scale into the same basis (enemy_view.gd:1867-1871 (in _update_shadow))
