...
3. **Behaviour the source could not have.** Because the shadow is a projection along
   ...
   (`presentation/combat/enemy_view.gd:2332` (`_update_shadow`); the swing itself
   enters at `presentation/combat/enemy_view.gd:4250` (`set_light_angle`)). No amount of tuning the CSS version
...
| `--foot-ox`, `--foot-oy` | Scanned off the painting's own alpha. Originally one point per creature — lowest opaque row, horizontal centroid of the band above it (`presentation/combat/enemy_view.gd:2150` (`_read_contact`)). Now the *origin* only: the contact is read per painting column, because one contact line cannot serve a creature standing on four feet at four heights. See [A flat billboard has one depth](../ui-bugs/flat-billboard-shadow-had-one-ground-line.md) |
...
(`presentation/combat/enemy_view.gd:2332` (`_update_shadow`)), so it buys a shadow that
...
(`presentation/combat/enemy_view.gd:1853` (in `_build_stage`)) gives a horizontal run of roughly 1.6 body
heights. That is
...
(`presentation/combat/enemy_view.gd:2141-2142` (`CAST_MIN`)):
...
- `assets/art/enemies/char-meta.json` — the ported per-character table. All of its
  ...
  data; `dy` is read as a resting height (`presentation/combat/enemy_view.gd:2781` (`_read_hover`)).
