...
(`presentation/combat/enemy_view.gd:2048` (`SHADOW_SHADER`)). Looking at the
...
- **The contact row sat below the feet.** The hinge was
  ...
  roster's largest (`presentation/combat/enemy_view.gd:787` (`_contact_u`)). A
...
- **The ground was one line, and could not be otherwise.** A flat billboard has
  ...
  (`presentation/combat/enemy_view.gd:806` (`_ground_tex`)), so planting the near
...
- **`shadow.dy` was doing half its job.** `dy` is the one per-creature knob that
  ...
  resting height (`presentation/combat/enemy_view.gd:2754` (`_read_hover`)). It
...
(`presentation/combat/enemy_view.gd:2123` (`CAST_MIN`)). Applying ×1.137 recovered
...
(`presentation/combat/enemy_view.gd:2252` (`_build_shadow`)) and the node position
puts that origin back on the body's own foot line
(`presentation/combat/enemy_view.gd:2314` (`_update_shadow`)):
...
`_read_ground` (`presentation/combat/enemy_view.gd:2199` (`_read_ground`)) takes the lowest opaque
...
across everything else (`presentation/combat/enemy_view.gd:2236` (`_interp`)). A
...
(`presentation/combat/enemy_view.gd:2224` (`_flat_ground`)), which is the
...
(`presentation/combat/enemy_view.gd:2252` (`_build_shadow`)) — a column of
vertices per ground sample — and the shear the `Basis` used to carry becomes
uniforms consumed per vertex
(`presentation/combat/enemy_view.gd:2048` (`SHADOW_SHADER`)):
...
```gdscript
const CONTACT_BAND: float = 0.15   # presentation/combat/enemy_view.gd:2183 (CONTACT_BAND)
const GROUND_N: int = 64           # presentation/combat/enemy_view.gd:2185 (GROUND_N)
```
...
(`presentation/combat/enemy_view.gd:2183` (`CONTACT_BAND`)) so a later reader can
...
**0.68 / 1.31** (`presentation/combat/enemy_view.gd:2123` (`CAST_MIN`); the prior
...
(`presentation/combat/enemy_view.gd:2754` (`_read_hover`)), lowers the ground
