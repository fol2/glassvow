...
`presentation/stage/layout_book.gd:207` (`FORMS`) declares the schema's field order as
...
- **Blaming `has()`.** The session's first diagnosis — and the code comment it
  left behind in the docstring of `presentation/stage/layout_book.gd:790`
  (`fields`) — was "`has()` lies
...
- **Do not declare a `const` Dictionary with a packed-array value type.** Use
  ...
  one such declaration, `presentation/stage/layout_book.gd:207` (`FORMS`); every other
...
- The docstring of `presentation/stage/layout_book.gd:790` (`fields`) — the
