...
`presentation/stage/layout_book.gd:292` (`FORMS`) declares the schema's field order as
...
- **Blaming `has()`.** The session's first diagnosis — and the code comment it
  left behind in the docstring of `presentation/stage/layout_book.gd:882`
  (`fields`) — was "`has()` lies
...
inline `FORMS[form]` are at `presentation/stage/layout_book.gd:444` (in `place`)
and `presentation/stage/layout_book.gd:822` (in `_defaults`).
...
- **Do not declare a `const` Dictionary with a packed-array value type.** Use
  ...
  one such declaration, `presentation/stage/layout_book.gd:292` (`FORMS`); every other
...
- The docstring of `presentation/stage/layout_book.gd:882` (`fields`) — the
