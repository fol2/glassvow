...
art alpha — today `presentation/combat/enemy_view.gd:3867` (`_alpha_at`) — behind
two methods:
...
`oversample` = 2.0 (`enemy_view.gd:331` (`oversample`)) one body unit is ≈ 230
...
computed for the floater (`enemy_view.gd:3031` (`body_centre`)); `dir` from the
...
- ~~**Whether the ward-shatter `crack()` call survives.**~~ **Closed, and it was a
  ...
  creature's body — and `combat_screen.gd:2555` (`_hit_enemy`), ordinary damage. Recorded rather than
