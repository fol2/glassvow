...
art alpha — today `presentation/combat/enemy_view.gd:3963` (`_alpha_at`) — behind
two methods:
...
`oversample` = 2.0 (`enemy_view.gd:344` (`oversample`)) one body unit is ≈ 230
...
computed for the floater (`enemy_view.gd:3124` (`body_centre`)); `dir` from the
...
| 4 | depth | **derived**, `GLASS_THICK × aperture(s)/aperture(0)`. `GLASS_THICK` already exists (`enemy_view.gd:100` (`GLASS_THICK`)) |
...
- ~~**Whether the ward-shatter `crack()` call survives.**~~ **Closed, and it was a
  ...
  model. The two calls are `combat_screen.gd:2432` (in `_handle_event`), which is the **`SHATTER` event** — the
  FACET gauge filling, `CONCEPTS.md` › Facet, and a wholly legitimate glass event on the
  creature's body — and `combat_screen.gd:2715` (`_hit_enemy`), ordinary damage. Recorded rather than
...
- **Two silhouette readers coexist, and the prediction here was wrong.** This bullet used
  ...
  (`presentation/combat/enemy_view.gd:3883` (in `_death_cells`)), because a shard
