...
art alpha — today `presentation/combat/enemy_view.gd:3829` (`_alpha_at`) — behind
two methods:
...
`oversample` = 2.0 (`enemy_view.gd:317` (`oversample`)) one body unit is ≈ 230
...
computed for the floater (`enemy_view.gd:2993` (`body_centre`)); `dir` from the
...
- ~~**Whether the ward-shatter `crack()` call survives.**~~ **Closed, and it was a
  ...
  creature's body — and `combat_screen.gd:2611` (`_hit_enemy`), ordinary damage. Recorded rather than
...
- **Two silhouette readers coexist, and the prediction here was wrong.** This bullet used
  ...
  (`presentation/combat/enemy_view.gd:3779` (in `_death_cells`)), because a shard
