...
(`presentation/combat/enemy_view.gd:2474` (`_voronoi`)):

| | standing crack web | death shatter |
|---|---|---|
| entry | `crack()` (`enemy_view.gd:2675` (`crack`)) | `mark_dead()` (`enemy_view.gd:2893` (`mark_dead`)) |
| sites | `_sites`, appended one per call, cap `MAX_SITES` | `_death_sites()` — graded rings around the burst |
| cells | `_cells()` (`enemy_view.gd:2503` (`_cells`)) — **`reach` > 0** | `_death_cells()` (`enemy_view.gd:3016` (`_death_cells`)) — **`reach` = 0** |
| body mask | none | `_touches_art()` (`enemy_view.gd:3069` (`_touches_art`)) |
...
every cell against `_disc(sites[i], reach)` (`enemy_view.gd:2461` (`_disc`)) — a
...
(`enemy_view.gd:2518` (`_prism`)) extrudes *every* edge of the cell into a side
...
(`enemy_view.gd:2590` (`_rebuild_glass`)) returns early below two sites, so the
...
and `_death_sites()` (`enemy_view.gd:2928` (`_death_sites`)) *already does this*,
