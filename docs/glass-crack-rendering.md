...
(`presentation/combat/enemy_view.gd:2338` (`_voronoi`)):

| | standing crack web | death shatter |
|---|---|---|
| entry | `crack()` (`enemy_view.gd:2498` (`crack`)) | `mark_dead()` (`enemy_view.gd:2699` (`mark_dead`)) |
| sites | `_sites`, appended one per call, cap `MAX_SITES` | `_death_sites()` — graded rings around the burst |
| cells | `_cells()` (`enemy_view.gd:2367` (`_cells`)) — **`reach` > 0** | `_death_cells()` (`enemy_view.gd:2822` (`_death_cells`)) — **`reach` = 0** |
| body mask | none | `_touches_art()` (`enemy_view.gd:2875` (`_touches_art`)) |
...
every cell against `_disc(sites[i], reach)` (`enemy_view.gd:2325` (`_disc`)) — a
...
(`enemy_view.gd:2382` (`_prism`)) extrudes *every* edge of the cell into a side
...
(`enemy_view.gd:2418` (`_rebuild_glass`)) returns early below two sites, so the
...
and `_death_sites()` (`enemy_view.gd:2734` (`_death_sites`)) *already does this*,
