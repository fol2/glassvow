...
(`presentation/combat/enemy_view.gd:3348` (`_voronoi`)):

| | standing crack web | death shatter |
|---|---|---|
| entry | `crack()` (`enemy_view.gd:3549` (`crack`)) | `mark_dead()` (`enemy_view.gd:3770` (`mark_dead`)) |
| sites | `_sites`, appended one per call, cap `MAX_SITES` | `_death_sites()` — graded rings around the burst |
| cells | `_cells()` (`enemy_view.gd:3377` (`_cells`)) — **`reach` > 0** | `_death_cells()` (`enemy_view.gd:3893` (`_death_cells`)) — **`reach` = 0** |
| body mask | none | `_touches_art()` (`enemy_view.gd:3946` (`_touches_art`)) |
...
every cell against `_disc(sites[i], reach)` (`enemy_view.gd:3335` (`_disc`)) — a
...
(`enemy_view.gd:3392` (`_prism`)) extrudes *every* edge of the cell into a side
band tagged `COLOR.r = 1`, and `GLASS_SHADER`
(`enemy_view.gd:1559` (`GLASS_SHADER`)) lights that band by Fresnel: `ALBEDO` mixes toward white,
...
(`enemy_view.gd:3464` (`_rebuild_glass`)) returns early below two sites, so the
...
and `_death_sites()` (`enemy_view.gd:3805` (`_death_sites`)) *already does this*,
