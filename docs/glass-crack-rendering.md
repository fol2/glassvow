...
(`presentation/combat/enemy_view.gd:3075` (`_voronoi`)):

| | standing crack web | death shatter |
|---|---|---|
| entry | `crack()` (`enemy_view.gd:3276` (`crack`)) | `mark_dead()` (`enemy_view.gd:3497` (`mark_dead`)) |
| sites | `_sites`, appended one per call, cap `MAX_SITES` | `_death_sites()` — graded rings around the burst |
| cells | `_cells()` (`enemy_view.gd:3104` (`_cells`)) — **`reach` > 0** | `_death_cells()` (`enemy_view.gd:3620` (`_death_cells`)) — **`reach` = 0** |
| body mask | none | `_touches_art()` (`enemy_view.gd:3673` (`_touches_art`)) |
...
every cell against `_disc(sites[i], reach)` (`enemy_view.gd:3062` (`_disc`)) — a
...
(`enemy_view.gd:3119` (`_prism`)) extrudes *every* edge of the cell into a side
band tagged `COLOR.r = 1`, and `GLASS_SHADER`
(`enemy_view.gd:1435` (`GLASS_SHADER`)) lights that band by Fresnel: `ALBEDO` mixes toward white,
...
(`enemy_view.gd:3191` (`_rebuild_glass`)) returns early below two sites, so the
...
and `_death_sites()` (`enemy_view.gd:3532` (`_death_sites`)) *already does this*,
...
accumulating toward `MAX_SITES` (`enemy_view.gd:104` (`MAX_SITES`)) = 32. So the
