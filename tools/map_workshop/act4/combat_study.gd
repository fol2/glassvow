...
func _run() -> void:
	...
	assert(combat != null and combat.act == 3)
	paused = true
	...
	assert(root.get_texture().get_image().save_png("/tmp/act4-combat-"+size_label+"-before.png")==OK)
	ground.material = final_material
	ground.queue_redraw()
	await process_frame
	await RenderingServer.frame_post_draw
	assert(root.get_texture().get_image().save_png("/tmp/act4-combat-"+size_label+"-after.png")==OK)
	print("GROUND_STUDY single_frozen_scene=true replaced_plates=1 act=3 size=",root.size)
	quit()
