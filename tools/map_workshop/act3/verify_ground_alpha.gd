...
func _run() -> void:
	...
	for y: int in range(viewport.size.y):
		for x: int in range(viewport.size.x):
	...
	print("OBSIDIAN_ALPHA ",JSON.stringify({"changed_alpha_pixels":changed_alpha,"solid":solid,"transparent":transparent,"pixels":viewport.size.x*viewport.size.y}))
	quit(0 if changed_alpha==0 and solid>1000 and transparent>100 else 1)
