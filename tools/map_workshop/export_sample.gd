...
func _run() -> void:
	for argument: String in OS.get_cmdline_user_args():
		if argument.begins_with("--act="):
			...
			quit(2)
			return
	if act<0 or act>3 or output.is_empty() or (production_journey and act not in [0,1,2,3]) or (not candidate_overrides.is_empty() and not first_attempt_only):
		quit(2)
		return
...
func _sample(content: ContentDB, act: int, seed_value: int) -> Dictionary:
	...
	if production_journey:
		if not spatial_recipe.is_empty():
			...
			scene.free()
			return {}
		var prepared: Dictionary = preload("res://presentation/map/map_journey_recipe.gd").build(nodes,edges,quality) if act==0 else preload("res://presentation/map/chapters/act2/recipe.gd").new().build(nodes,edges,quality) if act==1 else preload("res://presentation/map/chapters/act3/recipe.gd").new().build(nodes,edges,quality) if act==2 else preload("res://presentation/map/chapters/act4/recipe.gd").new().build(nodes,edges,quality)
