...
func _process(_delta: float) -> bool:
	...
	if _burst_done and _frame > WARMUP_FRAMES + 90 \
		...
		# Steady-state hold. The viewport's own GPU clock, like the actor probe.
		var vp_rid: RID = _stage.get_viewport_rid_for_bench()
		var us: float = RenderingServer.viewport_get_measured_render_time_gpu(vp_rid) \
