...
const DISPLAY_LEAVES: Dictionary = {
	...
	"itemName": true, "itemText": true, "poor": true, "ask": true,
...
## mirroring web `hydrateContent`). Main calls it at boot and at its atomic
## post-selection route boundary; returns the number of strings written.
##
...
static func _overlay_row(domain: String, src: Dictionary, row: Variant, undo: Array) -> int:
	...
	if domain == "cards" and src.has("textUp"):
		...
		if typeof(rest["textUp"]) == TYPE_STRING and typeof(up) == TYPE_DICTIONARY:
...
## dictionaries recurse, and arrays copy by index — strings directly (quest
## echoes, variant dialogue), dictionaries recursively (quest meetings).
	...
	for key_v: Variant in src:
		...
		if kind == TYPE_DICTIONARY and child != null:
			...
			for i: int in range(lines.size()):
				...
				if DISPLAY_STRING_ARRAYS.has(key) and typeof(line) == TYPE_STRING:
					written += _write(child, str(i), str(line), undo)
				elif DISPLAY_DICTIONARY_ARRAYS.has(key) and typeof(line) == TYPE_DICTIONARY:
					var target: Variant = _child(child, str(i))
					if target != null:
						var fields: Dictionary = line
						written += _overlay_node(fields, target, undo)
	return written
